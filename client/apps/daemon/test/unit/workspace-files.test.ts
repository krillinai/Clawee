...
describe('workspace file service', () => {
  ...
  it('rejects symlink escapes outside canonicalCwd', async () => {
    ...
    await expect(service.readContent({ threadId: 'thread_1', path: 'leak/secret.md' })).rejects.toMatchObject({
      code: 'PATH_ESCAPE'
    });
  ...
  unixIt('rejects saving through an in-workspace symlink and keeps target content unchanged', async () => {
    const { service } = createFixture({ sandbox: 'workspace-write' });
