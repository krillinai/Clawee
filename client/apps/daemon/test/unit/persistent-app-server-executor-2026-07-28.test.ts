...
describe('persistent app-server executor', () => {
  it('applies the server skill rule per new and resumed thread without affecting local runs', async () => {
    const fixture = createFixture();
    const inputs = [
      runInput({ runId: 'server-new', serverDeployment: true }),
      runInput({
        runId: 'server-resume',
        codexThreadId: 'codex-thread-existing',
        serverDeployment: true
      }),
      runInput({ runId: 'local-explicit', serverDeployment: false }),
      runInput({ runId: 'local-default' })
    ];

    for (const input of inputs) {
      const execution = fixture.executor.start(input);
      await execution.started;
      await execution.result;
    }

    const threadRequests = fixture.readMessages().filter(message =>
      message.method === 'thread/start' || message.method === 'thread/resume'
    );
    expect(threadRequests).toHaveLength(4);
    expect(threadRequests.slice(0, 2).every(request =>
      String((request.params as Record<string, unknown>).developerInstructions)
        .includes(SERVER_SKILL_CONFIDENTIALITY_INSTRUCTION)
    )).toBe(true);
    expect(threadRequests.slice(2).every(request =>
      !String((request.params as Record<string, unknown>).developerInstructions)
        .includes(SERVER_SKILL_CONFIDENTIALITY_INSTRUCTION)
    )).toBe(true);
  });
  ...
  it('waits for a reloaded MCP server to become ready before starting the turn', async () => {
    const fixture = createFixture();
    let knowledgeReady = false;
    let knowledgeReadyBeforeTurn = false;
    const run = fixture.executor.start(runInput({
      runId: 'run-wait-mcp-ready',
      model: 'delay-mcp-startup',
      onNotification(notification) {
        if (
          notification.method === 'mcpServer/startupStatus/updated'
          && (notification.params as Record<string, unknown> | undefined)?.name
            === 'enterprise_knowledge'
          && (notification.params as Record<string, unknown> | undefined)?.status
            === 'ready'
        ) {
          knowledgeReady = true;
        }
      ...
      onTurnStartWritten() {
        knowledgeReadyBeforeTurn = knowledgeReady;
      }
    }));
    ...
    expect(knowledgeReadyBeforeTurn).toBe(true);
  });

  it('waits for MCP startup on a new thread even when no reload is needed', async () => {
    const fixture = createFixture();
    const first = fixture.executor.start(runInput({
      runId: 'run-prime-manifest'
    }));
    await first.started;
    await first.result;
    ...
    const second = fixture.executor.start(runInput({
      runId: 'run-new-thread-wait-mcp',
      model: 'delay-mcp-thread-startup',
      onNotification(notification) {
        if (
          notification.method === 'mcpServer/startupStatus/updated'
          && (notification.params as Record<string, unknown> | undefined)?.name
  ...
  it('continues after an MCP server reports startup failure', async () => {
    const fixture = createFixture();
    let failureSeen = false;
    let failureSeenBeforeTurn = false;
    const run = fixture.executor.start(runInput({
      runId: 'run-mcp-failed',
      model: 'fail-mcp-startup',
      onNotification(notification) {
        if (
          notification.method === 'mcpServer/startupStatus/updated'
          && (notification.params as Record<string, unknown> | undefined)?.status
      ...
      onTurnStartWritten() {
        failureSeenBeforeTurn = failureSeen;
      }
    }));
    ...
    expect(failureSeenBeforeTurn).toBe(true);
  });

  it('continues after the MCP startup wait times out', async () => {
    const fixture = createFixture();
    const startedAt = Date.now();
    const run = fixture.executor.start(runInput({
      runId: 'run-mcp-timeout',
      model: 'cancel-mcp-startup',
      mcpStartupWaitTimeoutMs: 40
    }));
  ...
  it('runs up to ten app-server slots in parallel and reuses released slots', async () => {
    const hosts = Array.from({ length: 10 }, (_, index) =>
      fakeHost(500 + index, Promise.resolve())
    );
    const createHost = vi.fn(() => hosts[createHost.mock.calls.length - 1]!);
    const injections: AgentToolProcessInjection[] = [];
    const processInjector: AgentScheduleProcessInjector = {
      create: vi.fn(() => {
        const injection: AgentToolProcessInjection = {
          mcpServers: [],
          env: {},
          activate: vi.fn(() => ({ manifestKey: 'conversation' })),
          deactivate: vi.fn(),
          close: vi.fn()
        };
    ...
    const active = Array.from({ length: 10 }, (_, index) => executor.start(runInput({
      runId: `run-parallel-${index + 1}`,
      thread: thread({ id: `thread-parallel-${index + 1}` })
    })));
    ...
    expect(() => executor.start(runInput({
      ...
      thread: thread({ id: 'thread-over-limit' })
    }))).toThrow('busy');

    const started = await Promise.all(active.map(execution => execution.started));
    await Promise.all(active.map(execution => execution.result));
    expect(new Set(started.map(item => item.pid)).size).toBe(10);
    expect(createHost).toHaveBeenCalledTimes(10);
    expect(processInjector.create).toHaveBeenCalledTimes(10);
    expect(injections).toHaveLength(10);
    for (const [index, injection] of injections.entries()) {
      expect(injection.activate).toHaveBeenCalledWith(expect.objectContaining({
        runId: `run-parallel-${index + 1}`,
        thread: expect.objectContaining({ id: `thread-parallel-${index + 1}` })
      }));
      expect(injection.deactivate).toHaveBeenCalledWith(`run-parallel-${index + 1}`);
    }
    expect(executor.isBusy()).toBe(false);

    const reused = executor.start(runInput({
      runId: 'run-after-slot-release',
      thread: thread({ id: 'thread-after-slot-release' })
    }));
    expect(await reused.started).toEqual({ pid: 500, reused: true });
    await reused.result;
    expect(createHost).toHaveBeenCalledTimes(10);
    expect(injections[0]!.activate).toHaveBeenCalledWith(expect.objectContaining({
      runId: 'run-after-slot-release',
      thread: expect.objectContaining({ id: 'thread-after-slot-release' })
    }));
  ...
  unixIt('crosses the no-replay boundary only after turn/start is written', async () => {
    const fixture = createFixture();
    const onTurnStartWritten = vi.fn();
  ...
  it('forwards the Runtime writable-request gate to the persistent host', async () => {
    const host = fakeHost(305, Promise.resolve());
    const executor = createPersistentAppServerExecutor({
      codexBin: 'unused',
      codexHome: 'unused',
      createHost: vi.fn(() => host)
    ...
    executors.push(executor);
    const onBeforeWritableRequest = vi.fn(async () => undefined);

    const run = executor.start(runInput({
      runId: 'run-runtime-commit',
      onBeforeWritableRequest
    }));
    await run.started;
    await run.result;
...
rl.on('line', line => {
  ...
  if (message.method === 'thread/start' || message.method === 'thread/resume') {
    ...
    if (
      currentModel === 'delay-mcp-startup'
      || currentModel === 'delay-mcp-thread-startup'
      || currentModel === 'fail-mcp-startup'
      || currentModel === 'cancel-mcp-startup'
    ) {
      send({
    ...
    send({ id: message.id, result: { thread: { id: currentThreadId } } });
    if (currentModel === 'delay-mcp-thread-startup') {
      setTimeout(() => send({
        method: 'mcpServer/startupStatus/updated',
        params: {
          threadId: currentThreadId,
  ...
  if (message.method === 'config/mcpServer/reload') {
    if (
      ...
      if (currentModel === 'delay-mcp-startup') {
        setTimeout(() => send({
          method: 'mcpServer/startupStatus/updated',
          params: {
            threadId: currentThreadId,
      ...
      } else if (currentModel === 'fail-mcp-startup') {
        setTimeout(() => send({
          method: 'mcpServer/startupStatus/updated',
          params: {
            threadId: currentThreadId,
