...
describe('runtime storage', () => {
  ...
  it('backfills project sort order when upgrading an existing database', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'clawee-storage-project-order-'));
    ...
    const insert = legacy.prepare(`
      INSERT INTO projects (
        id, name, cwd, profile, sandbox, status, updated_at
      ) VALUES (?, ?, ?, 'default', 'follow-global', 'active', ?)
    `);
    insert.run('project_old', 'Old', join(tempDir, 'old'), '2026-08-01 00:00:00');
    insert.run('project_new', 'New', join(tempDir, 'new'), '2026-08-02 00:00:00');
    legacy.close();

    db = openRuntimeDatabase(databasePath);

    expect(columnNames(db, 'projects')).toContain('sort_order');
    expect(db.prepare(`
      SELECT id
      FROM projects
  ...
  it('persists mutable conversation configuration across database restarts', () => {
    ...
    expect(threads.getThread('thread_pinned')).toMatchObject({
      model: 'gpt-new',
      reasoning: 'high',
      pinned_at: '2026-08-06T00:00:00.000Z'
    });
  });
