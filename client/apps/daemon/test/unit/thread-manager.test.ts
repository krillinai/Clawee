...
describe('thread manager', () => {
  ...
  it('defaults conversations in follow-global projects to full access', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'clawee-thread-default-permission-'));
    const database = openTestDatabase(tempDir);
    const projectDir = join(tempDir, 'project');
    mkdirSync(projectDir, { recursive: true });
    const projects = createProjectManager({
    ...
    const project = projects.createProject({ cwd: projectDir });
    const manager = createThreadManager({
      db: database,
      dataDir: tempDir,
      projectManager: projects
    ...
    expect(project.sandbox).toBe('follow-global');
    expect(conversation.sandbox).toBe('danger-full-access');
  });
