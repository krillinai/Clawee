...
describe('codex mcp redaction', () => {
  ...
  it('redacts authorization credentials in JSON and TOML diagnostics', () => {
    expect(redactMcpText(
      '{"http_headers":{"Authorization":"Bearer json-secret"},"enabled":true}'
    )).toBe(
      '{"http_headers":{"Authorization":"Bearer [REDACTED]"},"enabled":true}'
    );
    expect(redactMcpText(
      'Authorization = "Bearer toml-secret"\nenabled = true'
    )).toBe(
