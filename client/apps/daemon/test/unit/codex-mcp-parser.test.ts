...
describe('codex mcp parser', () => {
  ...
  it('marks static and environment-backed HTTP headers as secrets', () => {
    const result = parseMcpGetOutput({
      name: 'enterprise',
      codexHome: '/tmp/codex-home',
      ...
      stdout: JSON.stringify({
        ...
        transport: {
          ...
          http_headers: {
            Authorization: 'Bearer agent-mcp-secret'
          },
          env_http_headers: {
            'X-Tenant': 'TENANT_ID'
          }
      ...
      stderr: 'connected with Bearer agent-mcp-secret',
      exitCode: 0
    });
    ...
    expect(JSON.stringify(result)).not.toContain('agent-mcp-secret');
