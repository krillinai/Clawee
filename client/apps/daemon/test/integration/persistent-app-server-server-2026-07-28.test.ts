...
describe('persistent app-server server integration', () => {
  ...
  it('restarts the persistent app-server after the model runtime changes', async () => {
    let notifyModelRuntimeChanged!: (reason: string) => void;
    const fixture = await createFixture({
      subscribeModelCatalogChanges(listener) {
        notifyModelRuntimeChanged = listener;
        return () => undefined;
      }
    });
    const threadId = await createThread();
    ...
    const first = await createRun(threadId, 'first');
    await waitForRun(first, 'succeeded');
    const firstPid = fixture.readDiagnostics(first).appServerPid;
    ...
    expect(fixture.readDiagnostics(second)).toMatchObject({
      appServerReused: false
    });
    expect(fixture.readDiagnostics(second).appServerPid).not.toBe(firstPid);
  });

  it('delays a model runtime restart until the active turn completes', async () => {
    let notifyModelRuntimeChanged!: (reason: string) => void;
    const fixture = await createFixture({
      subscribeModelCatalogChanges(listener) {
        notifyModelRuntimeChanged = listener;
    ...
    const threadId = await createThread();
    const active = await createRun(threadId, 'hold-schedule-active-turn');
    await waitForRun(active, 'running');
    await expect.poll(
      () => fixture.readSpawns().length,
      { timeout: 10_000 }
    ).toBe(1);
    const firstPid = fixture.readSpawns()[0]?.pid;

    notifyModelRuntimeChanged('model_catalog_changed');
    expect(await getRun(active)).toMatchObject({ status: 'running' });
    expect(fixture.readSpawns()).toHaveLength(1);

    writeFileSync(join(tempDir, 'release-schedule'), 'release', 'utf8');
    await waitForRun(active, 'succeeded');
    const next = await createRun(threadId, 'after-model-change');
    await waitForRun(next, 'succeeded');

    expect(fixture.readSpawns()).toHaveLength(2);
    expect(fixture.readDiagnostics(next)).toMatchObject({
      appServerReused: false
    });
    expect(fixture.readDiagnostics(next).appServerPid).not.toBe(firstPid);
  });

  it('runs ten manual conversations in parallel and queues the eleventh', async () => {
    const fixture = await createFixture();
    const projectId = await createProject();
    const threadIds: string[] = [];
    for (let index = 0; index < 11; index += 1) {
      threadIds.push(await createThread(projectId));
    }

    const activeRunIds: string[] = [];
    for (const [index, threadId] of threadIds.slice(0, 10).entries()) {
      activeRunIds.push(await createRun(threadId, `hold-schedule-${index + 1}`));
    }
    await expect.poll(() => fixture.readSpawns().length, { timeout: 10_000 }).toBe(10);

    const waitingRunId = await createRun(threadIds[10]!, 'hold-schedule-waiting');
    await expect.poll(async () => (await getRun(waitingRunId)).status).toBe('queued');
    expect(fixture.readSpawns()).toHaveLength(10);

    writeFileSync(join(tempDir, 'release-schedule'), 'release', 'utf8');
    await Promise.all([...activeRunIds, waitingRunId].map(runId =>
      waitForRun(runId, 'succeeded')
    ));
    expect(fixture.readSpawns()).toHaveLength(10);
  });
  ...
  it('waits for the Runtime commit before the server writes a Thread request', async () => {
    ...
    const threadId = await createThread();
    const runId = await createRun(threadId, 'commit-before-thread');

    await expect.poll(
      () => fixture.readMessages().map(message => message.method),
      { timeout: 5_000 }
    ...
    expect(fixture.readMessages().map(message => message.method))
...
async function createFixture(
  ...
  server = await buildServer({
    ...
    persistentAppServerMaxConcurrency: input.persistentAppServerMaxConcurrency,
    onBeforeCodexWritableRequest:
      input.onBeforeCodexWritableRequest,
    subscribeModelCatalogChanges: input.subscribeModelCatalogChanges,
...
async function createProject(): Promise<string> {
  ...
  return project.json().project.id as string;
}

async function createThread(projectId?: string): Promise<string> {
  const resolvedProjectId = projectId ?? await createProject();
...
rl.on('line', line => {
  ...
  if (message.method === 'thread/start' || message.method === 'thread/resume') {
    currentThreadId = message.params.threadId
      || ('codex-thread-' + process.pid + '-' + (++threadSequence));
