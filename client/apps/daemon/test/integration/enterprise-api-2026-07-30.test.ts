...
describe('enterprise runtime API', () => {
  it('authenticates gateway configuration and never restores credentials from another gateway', async () => {
    ...
    writeFileSync(configPath, 'gateway = "https://new.example"\n');
    const getMe = vi.fn(async () => activeMeResult());
    const store = createStore({ accessToken: 'old-gateway-token', expiresAt: '2099-01-01T00:00:00Z', origin: 'https://old.example' });
    server = await buildServer({
      token: 'secret', dataDir: tempDir, codexHome: join(tempDir, 'codex-home'),
      enterpriseConfigPath: configPath, enterpriseOrigin: 'https://new.example',
      enterpriseAgentIdentityStore: createAgentIdentityStore(),
      enterpriseCredentialStore: store, enterpriseHttpClient: createClient({ getMe })
    });
    await vi.waitFor(async () => {
      expect((await authRequest('GET', '/enterprise/session')).json().status).toBe('signed_out');
    });
    expect(getMe).not.toHaveBeenCalled();
    expect((await server.inject({ method: 'GET', url: '/enterprise/gateway' })).statusCode).toBe(401);
    expect((await server.inject({ method: 'POST', url: '/enterprise/gateway', payload: { gateway: 'https://other.example' } })).statusCode).toBe(401);
    expect((await authRequest('POST', '/enterprise/gateway', { gateway: 'https://other.example' })).statusCode).toBe(200);
    expect((await authRequest('GET', '/enterprise/gateway')).json().gateway).toBe('https://other.example');
    expect(readFileSync(configPath, 'utf8')).toContain('https://other.example');
  ...
  it('restores valid sessions without blocking local server startup', async () => {
    ...
    server = await buildServer({
      ...
      enterpriseActivityReporter: activityReporter,
    ...
    expect(activityReporter.resume).not.toHaveBeenCalled();
    expect(activityReporter.clear).toHaveBeenCalledWith('reporting_disabled');
  });

  it('resumes activity reporting when the enterprise enables it', async () => {
    tempDir = mkdtempSync(join(tmpdir(), 'clawee-enterprise-api-'));
    const activityReporter = createActivityReporter();
    server = await buildServer({
      token: 'secret',
    ...
    await vi.waitFor(() => {
      expect(activityReporter.resume).toHaveBeenCalledOnce();
    });
    expect(activityReporter.clear).not.toHaveBeenCalledWith('reporting_disabled');
  });
  ...
  it('uses the listening socket for DingTalk prepare and exempts only the exact GET callback', async () => {
    ...
    const exchange = vi.fn(async () => ({
      ...
      agentId,
      accessToken: 'dingtalk-enterprise-token',
      tokenType: 'Bearer' as const,
    ...
    const prepare = await fetch(`${origin}/enterprise/dingtalk/login/prepare`, {
      method: 'POST',
      headers: { Authorization: 'Bearer secret' }
    });
    expect(prepare.status).toBe(200);
    expect(prepare.headers.get('cache-control')).toBe('no-store');
    const localPort = new URL(origin).port;
    expect(prepared?.redirectUri).toBe(
      `http://127.0.0.1:${localPort}/enterprise/dingtalk/callback`
    );
    expect(await prepare.json()).toMatchObject({
      authorizationUrl: expect.stringContaining(prepared!.state)
    });

    const similar = await fetch(
      `${origin}/enterprise/dingtalk/callback/extra?state=${prepared!.state}`
    );
    expect(similar.status).toBe(401);
    const wrongMethod = await fetch(
      `${origin}/enterprise/dingtalk/callback?state=${prepared!.state}`,
      { method: 'POST' }
    );
    ...
    const malformedCallback = await fetch(
      `${origin}/enterprise/dingtalk/callback?code=invalid&state=${prepared!.state}`
    );
    expect(malformedCallback.status).toBe(400);
    expect(await malformedCallback.text()).toContain('参数不完整或格式无效');

    const mismatchedCallback = await fetch(
      `${origin}/enterprise/dingtalk/callback?code=${'c'.repeat(43)}&state=${'x'.repeat(43)}`
    );
    expect(mismatchedCallback.status).toBe(400);
    expect(await mismatchedCallback.text()).toContain('登录请求不匹配');
    expect(exchange).not.toHaveBeenCalled();

    const callback = await fetch(
      `${origin}/enterprise/dingtalk/callback?code=${'c'.repeat(43)}&state=${prepared!.state}`
    );
    expect(callback.status).toBe(200);
    expect(callback.headers.get('cache-control')).toBe('no-store');
    expect(callback.headers.get('referrer-policy')).toBe('no-referrer');
    const html = await callback.text();
    expect(html).toContain('钉钉登录成功');
    expect(html).not.toContain('dingtalk-enterprise-token');
    expect(html).not.toContain(prepared!.state);
    ...
    expect(await replay.text()).toContain('没有匹配的登录请求');
    expect(exchange).toHaveBeenCalledOnce();

    const retryPrepare = await fetch(`${origin}/enterprise/dingtalk/login/prepare`, {
      method: 'POST',
      headers: { Authorization: 'Bearer secret' }
    });
    expect(retryPrepare.status).toBe(200);
    const unavailableAccount = await fetch(
      `${origin}/enterprise/dingtalk/callback?error=not_enterprise_member&state=${prepared!.state}`
    );
    expect(unavailableAccount.status).toBe(400);
    expect(await unavailableAccount.text()).toContain('不是该企业的成员');
  });
...
function createClient(
  ...
  return {
    ...
    getMe: vi.fn(async () => activeMeResult()),
    logout: vi.fn(async () => undefined),
    revealAccountMcpToken: vi.fn(async () => ({
      tokenId: 'token_1',
      userId: 'acct_01JZ8W6A2M4S',
...
function activeMeResult(): EnterpriseMeResult {
  return {
    account: {
      subjectId: 'acct_01JZ8W6A2M4S',
      email: 'user@example.com',
