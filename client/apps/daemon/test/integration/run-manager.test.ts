...
function createTestRunManager(input: {
  ...
  onBeforeCodexWritableRequest?:
    Parameters<typeof createRunManager>[0]['onBeforeCodexWritableRequest'];
  ...
  const manager = createRunManager({
    ...
    onBeforeCodexWritableRequest: input.onBeforeCodexWritableRequest,
...
describe('run manager', () => {
  ...
  it('runs different threads in parallel while preserving same-thread ordering', async () => {
    ...
    expect(runB1.status).toBe('running');
    ...
    expect(persistent.starts).toHaveLength(2);
    ...
    expect(persistent.starts[2]?.codexThreadId).toBe(`codex-${runA1.id}`);
  ...
  it('queues the eleventh parallel thread with an explicit capacity message', async () => {
    ...
    const activeRuns = Array.from({ length: 10 }, (_, index) => {
      const thread = createPersistedThread(threadManager);
      return manager.startRun(threadRun(thread, `active-${index + 1}`));
    });
    await expect.poll(() => persistent.starts.length).toBe(10);

    const waitingThread = createPersistedThread(threadManager);
    const waiting = manager.startRun(threadRun(waitingThread, 'waiting'));

    expect(waiting).toMatchObject({ status: 'queued', queuePosition: 1 });
    await expect.poll(() => manager.listEvents(waiting.id).find(event =>
      event.type === 'diagnostic'
      && event.payload.type === 'diagnostic'
      && event.payload.code === 'RUN_CONCURRENCY_LIMIT'
    )).toMatchObject({
      payload: {
        message: '并行会话已达 10 个上限，当前任务正在等待可用执行位置',
        details: { maxConcurrency: 10, queuePosition: 1 }
      }
    });

    persistent.complete(activeRuns[0]!.id);
    await expect.poll(() => persistent.starts.length).toBe(11);
    expect(persistent.starts.at(-1)?.runId).toBe(waiting.id);

    for (const run of activeRuns.slice(1)) persistent.complete(run.id);
    persistent.complete(waiting.id);
    await Promise.all([...activeRuns, waiting].map(run =>
      waitForRunStatus(manager, run.id, 'succeeded')
    ));
  });

  it('cancels active and queued runs together', async () => {
    ...
    const active = manager.startRun(threadRun(threadA, 'active'));
    await expect.poll(() => persistent.starts.length).toBe(1);
    const queued = manager.startRun(threadRun(threadB, 'queued'));
    expect(queued.status).toBe('queued');

    manager.cancelAll();

    await waitForRunStatus(manager, active.id, 'canceled');
    await waitForRunStatus(manager, queued.id, 'canceled');
    expect(persistent.starts).toHaveLength(1);
  });

  it('runs a legacy knowledge thread through the ordinary persistent app-server queue', async () => {
    const persistent = createControllablePersistentExecutor();
    const { manager, threadManager } = createTestRunManager({
      runtimeTransport: 'app-server',
      persistentAppServerExecutor: persistent.executor
    ...
    const run = manager.startRun(threadRun(thread, '使用可用信息回答'));

    await expect.poll(() => persistent.starts.length).toBe(1);
    expect(persistent.starts[0]).toMatchObject({
      runId: run.id,
      thread: { id: thread.id }
    });
    persistent.complete(run.id);
    await waitForRunStatus(manager, run.id, 'succeeded');
  });

  it('injects enterprise knowledge routing into a resumed app-server turn', async () => {
    const persistent = createControllablePersistentExecutor();
    const { manager, threadManager } = createTestRunManager({
      runtimeTransport: 'app-server',
      persistentAppServerExecutor: persistent.executor
    ...
    const thread = createPersistedThread(threadManager, {
      codexThreadId: 'codex-thread-with-legacy-lark-context'
    });

    const run = manager.startRun(threadRun(thread, '再看下知识库'));

    await expect.poll(() => persistent.starts.length).toBe(1);
    expect(persistent.starts[0]).toMatchObject({
      codexThreadId: 'codex-thread-with-legacy-lark-context',
      prompt: expect.stringContaining('[Clawee Runtime 企业知识库路由]')
    });
    expect(persistent.starts[0]?.prompt).toContain(
      '禁止读取或调用 lark-wiki'
    );
    expect(persistent.starts[0]?.prompt).toContain('再看下知识库');
  ...
  it('waits for the Runtime commit before spawning an exec fallback run', async () => {
    ...
    const { manager, threadManager } = createTestRunManager({
      ...
      async onBeforeCodexWritableRequest() {
        steps.push('commit-started');
        await commit;
        steps.push('commit-finished');
      },
      async beforeRunSpawn() {
        steps.push('before-spawn');
      }
    });
    const thread = createPersistedThread(threadManager);

    const run = manager.startRun(threadRun(thread, 'commit-before-exec'));

    await expect.poll(() => steps).toEqual(['commit-started']);
  ...
  it('uses the context-enriched execution prompt and records only context references in metadata', async () => {
    ...
    expect(manager.getRun(run.id)?.publicPrompt).toBe('original user prompt');
  ...
  it('rotates an automatic schedule run once after resume fails and reseeds from the latest summary', async () => {
    ...
    await expect.poll(() => fake.readInvocationCount(), { timeout: 5_000 }).toBe(2);
  ...
  it('recovers an interactive auto resume when the rollout is missing', async () => {
    tempDir = mkdtempSync(join(tmpdir(), 'clawee-run-interactive-resume-recovery-'));
    const fake = createFakeCodex(tempDir, {
      stdoutLines: [],
      invocations: [
        {
          stdoutLines: [],
          stderrLines: ['no rollout found for thread id codex-thread-old'],
          exitCode: 1
        },
        {
          stdoutLines: [
    ...
    const run = await manager.createAndRun(threadRun(thread, '继续处理'));

    expect(run.status).toBe('succeeded');
    expect(fake.readInvocationCount()).toBe(2);
    expect(fake.readArgvs()[0]?.slice(0, 2)).toEqual(['exec', 'resume']);
    expect(fake.readArgvs()[1]).not.toContain('resume');
    expect(threadManager.getThread(thread.id)?.codexThreadId)
      .toBe('codex-thread-new');
  });

  it('does not recover an explicit interactive resume when the rollout is missing', async () => {
    tempDir = mkdtempSync(join(tmpdir(), 'clawee-run-explicit-interactive-resume-'));
    const fake = createFakeCodex(tempDir, {
      stdoutLines: [],
      invocations: [{
        stdoutLines: [],
    ...
    const run = await manager.createAndRun({
      ...threadRun(thread, '严格恢复'),
      resumeMode: 'resume_thread'
    });

    expect(run.status).toBe('failed');
    expect(fake.readInvocationCount()).toBe(1);
    expect(manager.getRun(run.id)?.errorCode).toBe('RESUME_TARGET_NOT_FOUND');
    expect(threadManager.getThread(thread.id)?.codexThreadId)
      .toBe('codex-thread-old');
  });

  it('recovers an app-server interactive auto resume when the rollout is missing', async () => {
    const persistent = createMissingResumePersistentExecutor();
    const { manager, threadManager } = createTestRunManager({
      runtimeTransport: 'app-server',
      persistentAppServerExecutor: persistent.executor
    });
    ...
    expect(run.status).toBe('succeeded');
    expect(persistent.starts).toHaveLength(2);
    expect(persistent.starts[0]?.codexThreadId).toBe('codex-thread-old');
    expect(persistent.starts[1]?.codexThreadId).toBeUndefined();
    expect(threadManager.getThread(thread.id)?.codexThreadId)
      .toBe('codex-thread-new');
  });

  it('does not recover an explicit app-server resume when the rollout is missing', async () => {
    const persistent = createMissingResumePersistentExecutor();
    const { manager, threadManager } = createTestRunManager({
      runtimeTransport: 'app-server',
      persistentAppServerExecutor: persistent.executor
  ...
  it('can cancel a running run', async () => {
    ...
    await waitForRunStatus(manager, run.id, 'canceled');
    expect(manager.listEvents(run.id).some(event => event.type === 'done')).toBe(true);
  });
});

function createControllablePersistentExecutor(maxConcurrency = 1): {
  ...
  const executor: PersistentAppServerExecutor = {
    ...
    async close() {
      for (const runId of activeRunIds) {
        completions.get(runId)?.resolve('interrupted');
      }
    }
  };
...
function createMissingResumePersistentExecutor(): {
  executor: PersistentAppServerExecutor;
  starts: PersistentAppServerExecutionInput[];
} {
  const starts: PersistentAppServerExecutionInput[] = [];
      ...
      if (starts.length === 1) {
        return {
          ...
          result: Promise.reject(new Error(
            ...
            + `no rollout found for thread id ${input.codexThreadId}`
          ))
        };
      }
      const result = (async () => {
        const threadId = 'codex-thread-new';
