...
const bundledCodexBin = join(
  process.cwd(),
  '../desktop/.pack/codex-runtime/bin',
...
describe.runIf(runRealCodex)('real codex smoke', () => {
  it('captures codex version', () => {
    const result = runSmokeCommand([bundledCodexBin, '--version']);
  ...
  it('captures codex exec help', () => {
    const result = runSmokeCommand([bundledCodexBin, 'exec', '--help']);
  ...
  it('captures codex mcp help', () => {
    const result = runSmokeCommand([bundledCodexBin, 'mcp', '--help']);
  ...
  it('verifies a Runtime-installed skill is accepted by the bundled codex environment', async () => {
    ...
    const dataDir = join(fixtureDir, `skills-runtime-${Date.now()}`);
  ...
  it('runs one schedule twice through the same Clawee and Codex threads', async () => {
    ...
    const server = await buildServer({
      ...
      capabilities: collectCodexCapabilityMatrix({
        codexBin: bundledCodexBin
      }),
