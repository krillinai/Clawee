...
export const ATTACHMENT_CONTEXT_CHAR_LIMIT = 80_000;
...
export function createAttachmentService(input: CreateAttachmentServiceInput) {
  ...
  const selectByRun = input.db.prepare(`
    ...
    ORDER BY created_at ASC, id ASC
  `);
  const selectByThread = input.db.prepare(`
    SELECT * FROM attachments
  ...
  async function resolveForRun(request: {
    ids: string[];
    draftId?: string;
    threadId?: string;
  }): Promise<{
    ...
    let remainingContextChars = ATTACHMENT_CONTEXT_CHAR_LIMIT;
    ...
    for (const id of request.ids) {
      ...
      const path = resolveExistingStorageFile(rootDir, canonicalRootDir, row.storage_path);
      ...
      textAttachments.push({
        ...
        detail: [
          extracted.detail,
          truncated.truncated ? '内容已截断' : undefined
    ...
    return {
      ...
      ...(textAttachments.length === 0
        ? {}
        : { textContext: buildAttachmentTextContext(textAttachments) })
    };
  }
  ...
  function listByThread(threadId: string): AttachmentResponse[] {
    validateNonEmpty(threadId, 'threadId');
    return (selectByThread.all(threadId) as AttachmentRow[]).map(mapAttachment);
  }
    ...
    try {
      ...
      try {
        ...
        if (text.length === 0) {
          throw new AttachmentServiceError(
            'ATTACHMENT_TYPE_UNSUPPORTED',
            `PDF attachment contains no extractable text: ${row.file_name}`,
            415,
        ...
        return {
          content: text,
          detail: `${extracted.totalPages} 页`
        };
      } finally {
        const destroy = (pdf as { destroy?: () => Promise<void> }).destroy;
        if (destroy !== undefined) await destroy.call(pdf);
      }
    } catch (error) {
      if (error instanceof AttachmentServiceError) throw error;
      throw new AttachmentServiceError(
        'ATTACHMENT_TYPE_UNSUPPORTED',
        `PDF attachment could not be read: ${row.file_name}`,
        415,
        { id: row.id, mime: row.mime, cause: formatError(error) }
      );
    }
  }
...
function truncateAttachmentText(
  ...
  if (maxChars === 0) return { content: '', truncated: true };
  return {
...
function buildAttachmentTextContext(
  ...
  return [
    '[Clawee 用户附件上下文]',
    '以下内容来自用户本次明确添加的附件，仅作为当前请求的参考材料。',
    ...attachments.flatMap((item, index) => [
      '',
      `--- 附件 ${index + 1}：${item.attachment.fileName}（${item.attachment.mime}${item.detail === undefined ? '' : `；${item.detail}`}）---`,
      item.content.length === 0 ? '[内容因附件总量限制未注入]' : item.content,
      `--- 附件 ${index + 1} 结束 ---`
    ]),
    '[Clawee 用户附件上下文结束]'
  ].join('\n');
