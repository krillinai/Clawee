...
export function migrate(db: Database.Database): void {
  ...
  const projectSortOrderAdded = ensureColumn(
    db,
    'projects',
    'sort_order',
    'sort_order INTEGER NOT NULL DEFAULT 0'
  );
  if (projectSortOrderAdded) {
    db.exec(`
      UPDATE projects AS current
      SET sort_order = (
        SELECT COUNT(*)
        FROM projects AS preceding
        WHERE preceding.updated_at > current.updated_at
          OR (
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_projects_status_sort_order
      ON projects(status, sort_order, updated_at DESC, id DESC);
