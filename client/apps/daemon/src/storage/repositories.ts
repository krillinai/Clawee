...
export type ProjectRepository = {
  ...
  findProjectsByName(name: string, status?: ProjectStatus | 'all'): ProjectRow[];
  ...
  updateProjectSortOrder(input: { id: string; sortOrder: number }): void;
...
export type ThreadRepository = {
  ...
  listCodexMigrationBindings(): Array<{
...
export function createProjectRepository(db: Database.Database): ProjectRepository {
  ...
  const findByName = db.prepare<{ name: string; status: ProjectStatus | 'all' }>(`
    SELECT *
    FROM projects
    WHERE name = @name AND (@status = 'all' OR status = @status)
  ...
  return {
    ...
    findProjectsByName(name, status = 'all'): ProjectRow[] {
      return findByName.all({ name, status }) as ProjectRow[];
    },
    ...
    updateProjectSortOrder(input): void {
      updateSortOrder.run(input);
    },
...
export function createThreadRepository(db: Database.Database): ThreadRepository {
  ...
  const updateThread = db.prepare(`
    ...
    SET title = CASE WHEN @titleSet = 1 THEN @title ELSE title END,
        model = CASE WHEN @modelSet = 1 THEN @model ELSE model END,
        reasoning = CASE WHEN @reasoningSet = 1 THEN @reasoning ELSE reasoning END,
  ...
  const listUnresolvedCodexBindingThreadIds = db.prepare(`
    ...
    ORDER BY threads.id ASC
  `);
  const listCodexMigrationBindings = db.prepare(`
    SELECT
      threads.id AS clawee_thread_id,
      threads.codex_thread_id
    FROM threads
    WHERE threads.codex_thread_id IS NOT NULL
    ORDER BY threads.id ASC
  `);
  ...
  return {
    ...
    updateThread(input): void {
      updateThread.run({
        ...
        modelSet: input.model === undefined ? 0 : 1,
        model: input.model ?? null,
        reasoningSet: input.reasoning === undefined ? 0 : 1,
    ...
    listCodexMigrationBindings(): Array<{
      ...
      return (listCodexMigrationBindings.all() as Array<{
        clawee_thread_id: string;
        codex_thread_id: string;
      }>).map(row => ({
        claweeThreadId: row.clawee_thread_id,
