...
export async function registerProjectRoutes(
  ...
  server.post<{ Body: unknown }>('/projects/directory-selection', async (request, reply) => {
    const body = parseProjectDirectorySelectionRequest(request.body);
    if (!body.ok) {
      return reply.code(400).send(apiError('VALIDATION_FAILED', body.message));
    }
    try {
      return { path: await directoryPicker.selectDirectory(body.value.purpose) };
    } catch (error) {
      return sendProjectDirectoryPickerError(reply, error);
    }
  });
  ...
  server.post<{ Body: unknown }>('/projects/reorder', async (request, reply) => {
    const body = parseReorderProjectsRequest(request.body);
    if (!body.ok) {
      return reply.code(400).send(apiError('VALIDATION_FAILED', body.message));
    }
    try {
      return { projects: manager.reorderProjects(body.value) };
    } catch (error) {
      return sendProjectError(reply, error);
    }
  });
...
function parseProjectDirectorySelectionRequest(
  body: unknown
): ParseResult<{ purpose: ProjectDirectorySelectionPurpose }> {
  if (!isPlainObject(body)) return { ok: false, message: 'body must be an object' };
  if (Object.keys(body).some(key => key !== 'purpose')) {
    return { ok: false, message: 'body contains unsupported fields' };
  }
  if (body.purpose === undefined) {
    return { ok: true, value: { purpose: 'add' } };
  }
  if (!isOneOf(body.purpose, ['add', 'create', 'replace'] as const)) {
    return { ok: false, message: 'purpose must be add, create, or replace' };
  }
  return { ok: true, value: { purpose: body.purpose } };
}
...
function parseCreateManagedProjectRequest(
  ...
  if (
    body.parentCwd !== undefined
    && (typeof body.parentCwd !== 'string' || body.parentCwd.trim().length === 0)
  ) {
    return { ok: false, message: 'parentCwd must be a non-empty string' };
  }
  if (Object.keys(body).some(key => key !== 'name' && key !== 'parentCwd')) {
    return { ok: false, message: 'body contains unsupported fields' };
  }
  return {
    ok: true,
    value: {
      name: body.name,
      ...(typeof body.parentCwd === 'string' ? { parentCwd: body.parentCwd } : {})
    }
  };
}

function parseReorderProjectsRequest(body: unknown): ParseResult<ReorderProjectsRequest> {
  if (!isPlainObject(body)) return { ok: false, message: 'body must be an object' };
  if (Object.keys(body).some(key => key !== 'projectIds')) {
    return { ok: false, message: 'body contains unsupported fields' };
  }
  if (!Array.isArray(body.projectIds)) {
    return { ok: false, message: 'projectIds must be an array' };
  }
  const projectIds: string[] = [];
  const uniqueIds = new Set<string>();
  for (const [index, projectId] of body.projectIds.entries()) {
    if (typeof projectId !== 'string' || projectId.trim().length === 0) {
      return {
        ok: false,
        message: `projectIds[${index}] must be a non-empty string`
      };
    }
    if (uniqueIds.has(projectId)) {
      return { ok: false, message: 'projectIds must not contain duplicates' };
    }
...
function sendProjectDirectoryPickerError(
  reply: FastifyReply,
  error: unknown
) {
  if (!(error instanceof ProjectDirectoryPickerError)) throw error;
  if (error.code === 'PROJECT_DIRECTORY_PICKER_UNAVAILABLE') {
    return reply.code(501).send(apiError(error.code, error.message));
  }
  if (error.code === 'PROJECT_DIRECTORY_PICKER_BUSY') {
    return reply.code(409).send(apiError(error.code, error.message));
  }
