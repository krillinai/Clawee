...
} from '../threads/run-event-history.js';
import type {
...
export type ThreadRoutePerformanceEvent = {
  component: 'thread_context';
  event: 'history_loaded' | 'runs_loaded';
  threadId: string;
  durationMs: number;
  itemCount: number;
  paginated: boolean;
...
export async function registerThreadRoutes(
  ...
  runManager: Pick<
    RunManager,
    'getLastEventSeq' | 'hasActiveRunForThread' | 'listRunsByThread'
  > & Partial<Pick<RunManager, 'listEvents'>>,
  options: {
  ...
  const reportPerformance = (event: ThreadRoutePerformanceEvent) => {
    try {
      options.onPerformanceEvent?.(event);
    } catch {
      // Diagnostics must not change thread API behavior.
    }
  };
  ...
  server.get('/threads/:id/runs', async (request, reply) => {
    ...
    reportPerformance({
      ...
      itemCount: response.runs.length,
      paginated: true
    });
    return response;
  });
  ...
  server.get('/threads/:id/history', async (request, reply) => {
    ...
    try {
      ...
      if (
        query.value.before === undefined
        && isRecoverableHistorySourceError(error)
      ) {
        history = {
          items: [],
          ...(paginationRequested ? { hasMore: false } : {})
    ...
    const threadRuns = runManager.listRunsByThread(id, 10_000);
    let items = attachScheduleRunMetadata(history.items, threadRuns);
    items = reconcileRunPromptsInThreadHistory(items, threadRuns);
    const eventRuns = query.value.before === undefined
      ? threadRuns.slice(0, query.value.limit ?? DEFAULT_HISTORY_LIMIT)
      : [];
    const attachmentsByRunId = new Map<string, AttachmentResponse[]>();
    for (const attachment of options.attachmentService?.listByThread(id) ?? []) {
      if (attachment.runId === undefined) continue;
      const runAttachments = attachmentsByRunId.get(attachment.runId) ?? [];
      runAttachments.push(attachment);
      attachmentsByRunId.set(attachment.runId, runAttachments);
    }
    items = attachRunAttachmentsToThreadHistory(
      items,
      threadRuns.map(run => ({
        id: run.id,
        publicPrompt: run.publicPrompt,
    ...
    if (
      query.value.before === undefined
      && runManager.listEvents !== undefined
    ) {
      const historyRunIds = eventRuns.map(run => ({
          id: run.id,
          publicPrompt: run.publicPrompt,
          attachments: attachmentsByRunId.get(run.id) ?? [],
          createdAt: run.createdAt,
      ...
      items = mergeRunEventsIntoThreadHistory(
        items,
        historyRunIds,
...
function isRecoverableHistorySourceError(error: unknown): boolean {
  if (
    error instanceof CodexAppServerResponseError
    && error.code === -32600
  ...
  return error instanceof Error
    && error.message === 'MODEL_SERVICE_CONFIGURATION_REQUIRED';
}
...
function parseUpdateThreadRequest(body: unknown): ParseResult<UpdateThreadRequest> {
  ...
  if (
    Object.keys(input).some(
      key => !['title', 'model', 'reasoning', 'sandbox', 'pinned'].includes(key)
    )
  ) {
