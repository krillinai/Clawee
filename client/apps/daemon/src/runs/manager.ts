...
const INTERACTIVE_RUN_TIMEOUT_MS = MCP_TASK_MAX_EXECUTION_MS;
...
export function createRunManager(options: RunManagerOptions): RunManager {
  ...
  const manager: RunManager = {
    startRun(input: CreateRunInput): CreatedRun {
      ...
      if (usePersistentAppServer) {
        ...
        const threadBlocked = runningThreadRun.has(thread.id);
        ...
        if (submissionMode === 'interrupt_and_enqueue') {
          ...
          const activeRunId = runningThreadRun.get(thread.id);
        ...
        const queuedSeq = nextSeqForRun(id);
        const statusWrite = publishStatus(id, queuedSeq, 'queued', publish, {
          threadId: runInput.threadId,
          codexThreadId
        });
        const capacityWrite = capacityBlocked && !threadBlocked
          ? publishDiagnostic(
              id,
              queuedSeq + 1,
              'RUN_CONCURRENCY_LIMIT',
              `并行会话已达 ${options.persistentAppServerExecutor!.maxConcurrency} 个上限，当前任务正在等待可用执行位置`,
              publish,
              {
                maxConcurrency: options.persistentAppServerExecutor!.maxConcurrency,
                queuePosition
              }
            )
          : Promise.resolve();
        void Promise.all([statusWrite, capacityWrite]).catch(() => undefined);
        return {
          id,
          threadId: runInput.threadId,
          status: 'queued',
    ...
    cancelAll() {
      ...
      for (const id of [...activeRuns.keys()]) manager.cancelRun(id);
    },
    ...
    steerRun(id: string): boolean {
      ...
      if (persistentIndex !== -1) {
        ...
        const threadId = queued.input.threadId;
        const activeRunId = threadId === undefined
          ? undefined
          : runningThreadRun.get(threadId);
        const activeRow = activeRunId === undefined
          ? undefined
          : runs.getRun(activeRunId);
        if (
    ...
    async close(closeOptions = {}) {
      if (closeWork === undefined) {
        ...
        closeWork = (async () => {
          ...
          firstError ??= reporterResult.find(
  ...
  function startExistingRun(input: QueuedRun): CreatedRun {
    ...
    if (usePersistentAppServer) runningPersistentRunIds.add(id);
    ...
    if (options.runtimeTransport === 'app-server') {
      ...
      function startAppServerAttempt() {
        const executionPrompt = resolveCodexExecutionPrompt(
          executionRunInput
        );
        ...
        > = {
          ...
          serverDeployment: options.serverDeployment,
          onBeforeWritableRequest:
            options.onBeforeCodexWritableRequest,
        ...
        return startCodexAppServer({
          ...
          env: mergeCodexEnvironment(options.env, agentToolInjection?.env),
      ...
      async function handleAppServerError(error: unknown): Promise<CreatedRun> {
        ...
        if (
          ...
          && (
            ...
            || (
              shouldRecoverMissingAutoResume(runInput)
              && isMissingResumeTargetFailure(errorMessage)
            )
          )
        ) {
    ...
    function startExecAttempt() {
      const beforeSpawn =
        options.onBeforeCodexWritableRequest === undefined
        && (
          thread === undefined
          || options.beforeRunSpawn === undefined
        )
          ? undefined
          : async () => {
              await options.onBeforeCodexWritableRequest?.();
              if (thread !== undefined) {
                await options.beforeRunSpawn?.({ runId: id, thread });
              }
            };
      return startCodexExec({
        ...
        cwd: executionRunInput.cwd,
        args: codexArgs!,
        prompt: resolveCodexExecutionPrompt(executionRunInput),
        env: mergeCodexEnvironment(options.env, agentToolInjection?.env),
        timeoutMs: runTimeouts.timeoutMs,
        spawnTimeoutMs: runTimeouts.spawnTimeoutMs,
        inactivityTimeoutMs: runTimeouts.inactivityTimeoutMs,
    ...
    async function handleExecResult(result: {
      ...
      if (
        ...
        && (
          shouldAutomaticallyRotateRun(runInput, thread)
          || (
            resumeFailureCode === 'RESUME_TARGET_NOT_FOUND'
            && shouldRecoverMissingAutoResume(runInput)
          )
        )
      ) {
  ...
  function insertInitialRun(
    ...
    runs.insertRun({
      ...
      publicPrompt: input.publicPrompt ?? null,
    ...
    try {
      options.activityReporter?.registerRun({
        runId: id,
        ...(input.threadId === undefined ? {} : { threadId: input.threadId }),
        prompt: input.createdBy === 'schedule'
          ? input.publicPrompt ?? input.prompt
          : input.prompt,
        createdBy: input.createdBy ?? 'api',
  ...
  function resolveCreateRunInput(input: CreateRunInput): ResolvedCreateRunInput {
    const normalizedInput: CreateRunInput = input.createdBy === 'schedule'
      ...
      : {
          ...input,
          publicPrompt: input.executionPrompt !== undefined
            && input.executionPrompt !== input.prompt
            ? input.prompt
            : undefined,
    ...
    if (normalizedInput.threadId !== undefined && options.threadAccess !== undefined) {
      ...
      return {
        ...
        model: Object.prototype.hasOwnProperty.call(normalizedInput, 'model')
          ? normalizedInput.model
          : thread.model ?? undefined,
        reasoning: Object.prototype.hasOwnProperty.call(normalizedInput, 'reasoning')
  ...
  function completeThreadRun(threadId: string | undefined, runId: string): void {
    runningPersistentRunIds.delete(runId);
  ...
  function startNextPersistentRuns(): void {
    const executor = options.persistentAppServerExecutor;
    if (closing || executor === undefined) return;
    while (runningPersistentRunIds.size < executor.maxConcurrency) {
...
function resolveCodexExecutionPrompt(
  input: ResolvedCreateRunInput
): string {
  return buildEnterpriseKnowledgeExecutionPrompt({
    publicPrompt: input.publicPrompt ?? input.prompt,
    executionPrompt: input.executionPrompt ?? input.prompt
  });
}
...
function classifyResumeFailure(stdoutLines: string[], stderr: string): 'RESUME_TARGET_NOT_FOUND' | 'RESUME_FAILED' {
  ...
  return output.includes('not found')
    || output.includes('no rollout found')
    || output.includes('no session')
    || output.includes('unknown session')
...
function isMissingResumeTargetFailure(message: string): boolean {
  const normalized = message.toLowerCase();
  return normalized.includes('no rollout found for thread id')
    || normalized.includes('no session found for thread id')
    || normalized.includes('unknown session');
}
...
export function errorToTerminationReason(error: unknown): TerminationReason {
  if (error instanceof CodexExecError) {
...
export function shouldAutomaticallyRotateRun(
  input: CreateRunInput,
  _thread: RuntimeThread | undefined
): boolean {
  if (
...
function shouldRecoverMissingAutoResume(input: CreateRunInput): boolean {
