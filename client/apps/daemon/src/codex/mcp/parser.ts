...
export function parseMcpListOutput(input: ParseMcpListOutputInput): CodexMcpListResponse {
  ...
  if (Array.isArray(parsed)) {
    ...
    const sensitiveValues = plainServers.flatMap(getServerSensitiveValues);
...
function mapServer(
  ...
  const staticHeaderValues = getRecordStringValues(
    transportConfig.http_headers
  );
  const envHeaderKeys = getRecordStringKeys(
    transportConfig.env_http_headers
  );
  const sensitiveValues = [
    ...getEnvValues(transportConfig.env),
    ...staticHeaderValues
  ];
  ...
  const result: CodexMcpServerResponse = {
    ...
    hasSecrets:
      envKeys.length > 0
      || staticHeaderValues.length > 0
      || envHeaderKeys.length > 0
      || redactionChanged,
...
function getServerSensitiveValues(server: Record<string, unknown>): string[] {
  const transportConfig = isPlainObject(server.transport)
    ? server.transport
    : server;
  return [
    ...getEnvValues(transportConfig.env),
    ...getRecordStringValues(transportConfig.http_headers)
  ];
}

function getRecordStringKeys(value: unknown): string[] {
...
function getRecordStringValues(value: unknown): string[] {
  if (!isPlainObject(value)) return [];
