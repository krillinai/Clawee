...
export type CodexSessionMigrationSourceCandidate = {
  codexThreadId: string;
  sourcePath: string;
  fileSize: number;
  mtimeMs: number;
...
export type CodexSessionIndexRepository = {
  ...
  ): CodexSessionMigrationSourceCandidate[];
...
export function createCodexSessionIndexRepository(
  ...
  return {
    ...
    listMigrationSourceCandidates(
      ...
      const ids = [...new Set(codexThreadIds)];
      ...
      for (let start = 0; start < ids.length; start += 500) {
        ...
        rows.push(...db.prepare(`
          SELECT
            ...
            source.file_size,
            source.mtime_ms,
            source.parsed_offset,
            source.last_error
          FROM codex_sessions session
          INNER JOIN codex_session_sources source
            ON source.path = session.source_path
          WHERE session.codex_thread_id IN (${placeholders})
          ORDER BY session.codex_thread_id ASC, source.path ASC
