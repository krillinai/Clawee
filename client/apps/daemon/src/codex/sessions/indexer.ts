...
export type CreateCodexSessionIndexerInput = {
  ...
  collections?: Array<'sessions' | 'archived_sessions'>;
...
export function createCodexSessionIndexer(
  ...
  return {
    sync(options = {}): CodexSessionIndexSyncResult {
      const collections = input.collections ?? ['sessions'];
      const files = collections
        .flatMap(collection => {
          const directory = join(input.codexHome, collection);
          return existsSync(directory) ? listJsonlFiles(directory) : [];
        })
