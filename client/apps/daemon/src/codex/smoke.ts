...
async function runCodexJsonTurn(input: {
  ...
  return parseSmokeTurn(
    [input.codexBin, ...input.args],
    exitCode,
    stdout,
    stderr,
    { timedOut, terminationSignal }
  );
}
...
export async function runRealCodexResumeSmoke(input: {
  codexBin: string;
  marker: string;
}): Promise<RealCodexResumeSmokeResult> {
