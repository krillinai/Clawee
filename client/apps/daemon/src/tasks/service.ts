...
export type TaskService = {
  ...
  getByRunId(runId: string): TaskItem | undefined;
};
...
export function createTaskService(options: {
  ...
  const listTasks = options.db.prepare<{
    ...
    LIMIT @limit
  `);
  const getTaskByRunId = options.db.prepare<{ runId: string }>(`
    SELECT
      r.*,
      t.title AS thread_title,
      s.name AS schedule_name,
  ...
  return {
    ...
    getByRunId(runId) {
      const row = getTaskByRunId.get({ runId }) as TaskRow | undefined;
      if (row === undefined) return undefined;
      return mapTask(
        row,
        options,
        loadScheduleFailureSummaries(options.db, [row]).get(row.id)
      );
    }
  };
}
