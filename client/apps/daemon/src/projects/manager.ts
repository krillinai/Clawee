...
export function createProjectManager(input: CreateProjectManagerInput): ProjectManager {
  ...
  return {
    ...
    createManagedProject(request: CreateManagedProjectRequest): ProjectResponse {
      ...
      const parentCwd = request.parentCwd === undefined
        ? ensureManagedProjectRoot()
        : requireAvailableDirectory(request.parentCwd, homeDir).canonicalCwd;

      const cwd = join(parentCwd, name);
      if (existsSync(cwd)) {
    ...
    findProjectsByName(
      ...
      return projects.findProjectsByName(name, status).map(mapProjectRow);
    },
    ...
    reorderProjects(request: ReorderProjectsRequest): ProjectResponse[] {
      const activeProjectIds = projects.listProjects('active').map(project => project.id);
      if (!hasSameProjectIds(activeProjectIds, request.projectIds)) {
        throw new ProjectManagerError(
          'PROJECT_ORDER_CONFLICT',
          'Project list changed before the new order could be saved'
        );
      }
      input.db.transaction(() => {
        request.projectIds.forEach((id, sortOrder) => {
          projects.updateProjectSortOrder({ id, sortOrder });
        });
      })();
      return projects.listProjects('active').map(mapProjectRow);
    },
  ...
  function ensureManagedProjectRoot(): string {
    try {
      mkdirSync(managedProjectRoot, { recursive: true });
      return managedProjectRoot;
    } catch {
      throw new ProjectManagerError(
        'PROJECT_DIRECTORY_UNAVAILABLE',
        'Clawee 默认项目目录不可用'
...
function hasSameProjectIds(currentIds: string[], requestedIds: string[]): boolean {
  if (currentIds.length !== requestedIds.length) return false;
  const requested = new Set(requestedIds);
  return requested.size === currentIds.length
