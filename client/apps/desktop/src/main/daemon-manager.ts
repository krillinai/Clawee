...
import {
  ...
  type CodexRuntimeLaunchContext
...
export class DaemonManager extends EventEmitter<DaemonManagerEvents> {
  ...
  private codexCleanupWork = Promise.resolve();

  constructor(
    private readonly logger: DesktopLogger,
    private readonly terminateCodexProcessTree: CodexProcessTreeTerminator =
      terminateCodexProcessTreeByPlatform
  ) {
    super();
  }
  ...
  start(input: DaemonStartInput): Promise<DaemonConnection> {
    ...
    this.startWork = this.codexCleanupWork
      .then(() => this.startInternal(input))
      .finally(() => {
  ...
  private startInternal(input: DaemonStartInput): Promise<DaemonConnection> {
    return new Promise((resolve, reject) => {
      ...
      this.logger.info('Daemon Utility Process starting', {
        ...
        runtimeId: input.runtime.candidate.runtimeId,
        runtimeSource: input.runtime.candidate.source,
        runtimeTarget: input.runtime.candidate.target,
        codexEntryPath: input.runtime.candidate.entryPath,
        codexHomePath: input.runtime.candidate.homePath,
      ...
      child.on('message', message => {
        ...
        if (parsed.type === 'clawee_runtime_performance') {
          this.logger.info('Runtime performance', parsed.details);
        } else if (parsed.type === 'codex_child_started') {
          this.codexPids.add(parsed.pid);
        } else {
          this.codexPids.delete(parsed.pid);
        }
      });
      ...
      child.once('exit', code => {
        ...
        this.codexCleanupWork = this.codexCleanupWork
          .then(() => this.terminateKnownCodexProcesses(true));
        if (!settled) {
          settled = true;
  ...
  private async terminateKnownCodexProcesses(force: boolean): Promise<void> {
    await Promise.all([...this.codexPids].map(async pid => {
      await this.terminateCodexProcessTree(pid, force);
    }));
...
export function buildDaemonEnvironment(
  ...
  const modelServiceKeys = new Set([
    'CLAWEE_MODEL_API_KEY'
  ]);
  for (const key of Object.keys(env)) {
    const normalized = key.toUpperCase();
    if (
      enterpriseKeys.has(normalized)
      || runtimeKeys.has(normalized)
      || modelServiceKeys.has(normalized)
    ) {
      delete env[key];
    }
  }
...
export function parseDaemonProcessMessage(
  ...
  if (
    isRecord(payload)
    && payload.type === 'clawee_runtime_performance'
