...
describe('Workspace Ready sender validation', () => {
  ...
  it('accepts only the fixed Vite origin in development', () => {
    expect(isWorkspaceUrl('http://127.0.0.1:19860/', true)).toBe(true);
    expect(isWorkspaceUrl('http://localhost:19860/', true)).toBe(false);
    expect(isWorkspaceUrl('http://127.0.0.1:19861/', true)).toBe(false);
  });
});
