...
describe('Runtime data migration', () => {
  it('copies a validated Runtime database into an empty target', () => {
    ...
    mkdirSync(join(source, 'private'));
    writeFileSync(
      join(source, 'private', 'credentials.json'),
      '{"schemaVersion":1,"modelService":{"apiKey":"must-not-copy"}}'
    );
    ...
    copied.close();
    expect(existsSync(join(target, 'note.txt'))).toBe(true);
