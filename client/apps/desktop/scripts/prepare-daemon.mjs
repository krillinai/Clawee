...
pruneDeploymentRoot();
const sourceSqliteRoot = realpathSync(
  resolve(rootDir, 'apps/daemon/node_modules/better-sqlite3')
);
const targetSqliteRoot = realpathSync(
  resolve(targetDir, 'node_modules/better-sqlite3')
);
const nodeGypCli = createRequire(
  join(sourceSqliteRoot, 'package.json')
...
assertExists(resolve(targetDir, 'node_modules/which/package.json'));
assertExists(resolve(targetDir, 'node_modules/path-key/package.json'));
assertExists(resolve(targetDir, 'node_modules/shebang-command/package.json'));
...
function pruneDeploymentRoot() {
  ...
  removeWorkspaceSelfReference(targetDir, '@clawee/daemon');
  rmSync(
