...
const appCss = readNormalizedText('src/styles/app.css');
const mainTsx = readNormalizedText('src/main.tsx');
const appControllerTsx = readNormalizedText('src/app/AppController.tsx');
const skillMarketCss = readNormalizedText('src/features/plugins/skill-market.css');
const schedulesCss = readNormalizedText('src/features/schedules/schedules-view.css');
const settingsCss = readNormalizedText('src/features/settings/settings-management.css');
const taskCenterCss = readNormalizedText('src/features/tasks/task-center.css');
const tokensCss = readNormalizedText('src/styles/tokens.css');
...
describe('app CSS visual contracts', () => {
  ...
  it('uses compact desktop control sizes and 44px mobile touch targets', () => {
    ...
    expect(appCss).toMatch(/@media \(max-width: 760px\)\s*\{[\s\S]*?\.sidebar-recent-row-shell,[\s\S]*?min-height:\s*var\(--control-touch\);/);
  });
  ...
  it('keeps sidebar project rows fixed while loading and places the scrollbar at the edge', () => {
    const sidebarContent = cssBlock('.sidebar-content');
    ...
    const projectConversationList = cssBlock('.sidebar-project-conversations');
    const projectConversationRow = cssBlock('.sidebar-project-conversation-row');
    const selectedConversationRow = cssBlock('.sidebar-recent-row[aria-current="page"]');
    ...
    expect(sidebarContent).toContain('grid-auto-rows: max-content;');
    expect(sidebarContent).toContain('overflow-y: auto;');
    expect(sidebarContent).toContain('margin-right: -12px;');
    expect(sidebarContent).toContain('padding-right: 12px;');
    expect(projectTree).toContain('padding-bottom: 4px;');
    expect(cssBlock('.sidebar-section')).toContain(
    ...
    expect(projectConversationList).toContain('display: grid;');
    expect(projectConversationList).toContain('border-left: 1px solid');
    expect(projectConversationRow).toContain('height: 30px;');
    expect(projectConversationRow).toContain('text-decoration: none;');
    expect(cssBlock('.conversation-row strong')).toContain('font-weight: 500;');
    expect(cssBlock('.sidebar-recent-row strong')).toContain('font-weight: 500;');
    expect(cssBlock('.sidebar-section h2')).toContain('font-weight: 500;');
    expect(appCss).toMatch(/\.conversation-row\s*\{[^}]*padding:\s*0 8px;/);
    ...
    expect(appCss).toContain(
      '.sidebar-conversation-row-shell:hover .sidebar-recent-row[aria-current="page"]'
    );
    expect(appCss).toContain(
      '.sidebar-conversation-row-shell:focus-within .sidebar-recent-row[aria-current="page"]'
    );
    ...
    expect(appCss).toMatch(
      /\.sidebar-conversation-row-shell:hover \.conversation-row,[^{]+\{[^}]*padding-right: 36px;[^}]*background:/
    );
    expect(cssBlock('.sidebar-conversation-actions')).toContain('width: 28px;');
    expect(cssBlock('.sidebar-conversation-actions')).toContain('grid-template-columns: 28px;');
    expect(cssBlock('.sidebar-conversation-actions')).toContain('position: absolute;');
    expect(cssBlock('.sidebar-conversation-actions.has-one-conversation-shortcut'))
      .toContain('grid-template-columns: repeat(2, 28px);');
    expect(cssBlock('.sidebar-conversation-actions.has-two-conversation-shortcuts'))
      .toContain('grid-template-columns: repeat(3, 28px);');
    ...
    expect(appCss).toMatch(
      /\.sidebar-conversation-row-shell\.has-two-conversation-shortcuts:hover \.conversation-row,[^{]+\{[^}]*padding-right: 92px;/
    );
    expect(appCss).not.toContain(
      '.sidebar-recent-row[aria-current="page"] + .sidebar-conversation-archive'
    );
  });
  ...
  it('keeps the model service setup spinner in the eagerly loaded app CSS', () => {
    const spinner = cssBlock('.model-service-setup__spinner');

    expect(spinner).toContain('flex: 0 0 auto;');
    expect(spinner).toContain(
      'animation: model-service-setup-spin 900ms linear infinite;'
    );
    expect(appCss).toMatch(
      /@keyframes model-service-setup-spin\s*\{[^}]*transform:\s*rotate\(360deg\);/
    );
    expect(settingsCss).not.toContain('.model-service-setup__spinner');
  });

  it('keeps recent rows flat, stable, and scrollable', () => {
    const recentSection = cssBlock('.sidebar-recent-section');
    const recentHeading = cssBlock('.sidebar-recent-heading');
    const recentToggle = cssBlock('.sidebar-recent-toggle');
    const recentList = cssBlock('.sidebar-recent-list');
    const recentRow = cssBlock('.sidebar-recent-row');

    expect(recentSection).toContain('align-content: start;');
    expect(recentHeading).toContain('grid-template-columns: minmax(0, 1fr) 28px;');
    expect(recentToggle).toContain('width: 28px;');
    expect(recentToggle).toContain('height: 28px;');
    expect(appCss).toMatch(/\.sidebar-recent-list\s*\{[^}]*align-content: start;/);
    expect(recentRow).toContain('height: 34px;');
    expect(recentRow).toContain('min-height: 34px;');
    expect(recentRow).toContain('grid-template-columns: minmax(0, 1fr) auto;');
    expect(cssBlock('.sidebar-content')).toContain('overflow-y: auto;');
    expect(appCss).toContain('.sidebar-recent-row-shell:hover .sidebar-recent-row');
    expect(appCss).not.toContain('.sidebar-recent-group');
    expect(appCss).not.toContain('.sidebar-task-action');
    expect(appCss).not.toContain('.sidebar-task-spinner');
  });
  ...
  it('uses local fonts and opaque primary surfaces during viewport changes', () => {
    ...
    expect(tokensCss).toContain(':root[data-platform="win32"]');
    expect(tokensCss).toContain('"Segoe UI Variable Text", "Segoe UI", "Noto Sans SC Variable", "Microsoft YaHei UI"');
    expect(tokensCss).toContain('--conversation-content-max: 768px;');
    expect(tokensCss).toContain(':root[data-platform="win32"][data-theme="dark"]');
    expect(tokensCss).toContain('--conversation-bg: #181818;');
    expect(tokensCss).toContain('--windows-assistant-text: #e3e3e3;');
    expect(mainTsx).toContain("import '@fontsource-variable/noto-sans-sc/wght.css';");
  ...
  it('keeps the enterprise login Mac baseline and scopes Windows typography', () => {
    expect(cssBlock('.enterprise-access-gate')).not.toContain('font-family: var(--font);');
    expect(cssBlock('.enterprise-auth-header h1')).toContain('font-size: 21px;');
    expect(cssBlock('.enterprise-auth-header p')).toContain('font-size: 13px;');
    expect(cssBlock('.enterprise-email-mode button')).toContain('font-size: 12px;');
    expect(cssBlock('.enterprise-email-field input')).toContain('font-size: 13px;');
    expect(cssBlock('.enterprise-auth-agreement')).toContain('font-size: 12px;');
    expect(cssBlock(':root[data-platform="win32"] .enterprise-access-gate')).toContain(
      'font-family: var(--font);'
    );
    expect(cssBlock(':root[data-platform="win32"] .enterprise-auth-header h1')).toContain(
      'font-size: 22px;'
    );
    expect(cssBlock(':root[data-platform="win32"] .enterprise-email-field input')).toContain(
      'font-size: 15px;'
    );
    expect(appCss).toMatch(
      /:root\[data-platform="win32"\] \.enterprise-auth-header p,[\s\S]*?:root\[data-platform="win32"\] \.enterprise-auth-card \.enterprise-account-secondary-action\s*\{[^}]*font-size:\s*14px;/
    );
  });
  ...
  it('uses one opaque fixed source bar above the shared skill catalog scroller', () => {
    const page = skillMarketCssBlock('.plugins-page');
    const header = skillMarketCssBlock('.plugins-source-header');
    const content = skillMarketCssBlock('.plugins-source-content');
    ...
    expect(skillMarketCss).not.toMatch(
      /@media \(min-width: 921px\)[\s\S]*?\.skill-market\s*\{[^}]*overflow-y:\s*auto;/
    );
  });
  ...
  it('uses a compact switch list for the Composer connector shortcut', () => {
    ...
    expect(cssBlock('.composer-connector-switch:disabled')).not.toContain('opacity:');
    expect(cssBlock('.composer-connector-switch[data-updating="true"]'))
  ...
  it('opens the Composer capability submenu upward beside the add menu', () => {
    const composerWrap = cssBlock('.composer-wrap');
    ...
    expect(submenu).toContain('top: auto;');
    expect(submenu).toContain('bottom: 0;');
  ...
  it('keeps user messages as bubbles and assistant replies as unframed prose', () => {
    ...
    const markdownProse = cssBlock('.markdown-prose');
    const assistantProse = cssBlock('.markdown-prose[data-variant="assistant"]');
    const inlineCode = cssBlock('.markdown-prose .md-inline-code');
    const bareLinkWithIcon = cssBlock('.markdown-prose .md-link-bare:has(.md-link-site-icon)');
    const bareLinkLabel = cssBlock(
      '.markdown-prose .md-link-bare:has(.md-link-site-icon) .md-link-label'
    );
    const windowsProse = cssBlock(':root[data-platform="win32"] .markdown-prose');
    const windowsAssistant = cssBlock(
      ':root[data-platform="win32"] .markdown-prose[data-variant="assistant"]'
    );
    const windowsStrong = cssBlock(
      ':root[data-platform="win32"] .markdown-prose[data-variant="assistant"] strong'
    );
    const windowsInlineCode = cssBlock(
      ':root[data-platform="win32"] .markdown-prose .md-inline-code'
    );
    ...
    expect(markdownProse).not.toContain('font-family: var(--font);');
    expect(assistantProse).toContain('font-size: 14px;');
    expect(assistantProse).toContain('line-height: 1.68;');
    expect(inlineCode).toContain('padding: 1px 4px;');
    expect(inlineCode).toContain('font-size: 0.9em;');
    expect(bareLinkWithIcon).toContain('max-width: 100%;');
    expect(bareLinkWithIcon).toContain('display: inline-flex;');
    expect(bareLinkWithIcon).not.toContain('grid-template-columns:');
    expect(bareLinkLabel).toContain('min-width: 0;');
    expect(bareLinkLabel).toContain('overflow-wrap: anywhere;');
    expect(windowsProse).toContain('font-family: var(--font);');
    expect(windowsProse).toContain('font-weight: 400;');
    expect(windowsProse).toContain('font-optical-sizing: auto;');
    expect(windowsProse).toContain('font-synthesis: none;');
    expect(windowsProse).toContain('letter-spacing: 0;');
    expect(windowsAssistant).toContain('color: var(--windows-assistant-text);');
    expect(windowsAssistant).toContain('font-size: 14px;');
    expect(windowsAssistant).toContain('line-height: 1.64;');
    expect(windowsStrong).toContain('color: inherit;');
    expect(windowsStrong).toContain('font-weight: 600;');
    expect(windowsInlineCode).toContain('padding: 1px 5px;');
    expect(windowsInlineCode).toContain('font-size: 0.875em;');
    expect(appCss).toContain(
      ':root[data-platform="win32"] .markdown-prose[data-variant="assistant"] .md-p + .md-p'
    );
    expect(appCss).toMatch(
      /:root\[data-platform="win32"\] \.markdown-prose \.md-ul li,\n:root\[data-platform="win32"\] \.markdown-prose \.md-ol li\s*\{[^}]*margin:\s*3px 0;/
    );
