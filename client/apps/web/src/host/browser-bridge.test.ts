...
describe('browserBridge', () => {
  ...
  it('accepts authenticated Server Mode same-origin configs', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () =>
        new Response(JSON.stringify({ baseUrl: '', token: 'server-token' }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' }
        })
      )
  ...
  it('opens external HTTPS pages and reports browser popup blocking', async () => {
    const opened = {} as Window;
    const open = vi.spyOn(window, 'open').mockReturnValueOnce(opened)
      .mockReturnValueOnce(null);

    await expect(browserBridge.openExternal('https://billing.example/recharge'))
      .resolves.toBeUndefined();
    expect(open).toHaveBeenCalledWith(
      'https://billing.example/recharge',
      '_blank',
      'noopener,noreferrer'
    );
    await expect(browserBridge.openExternal('https://billing.example/recharge'))
      .rejects.toThrow('BROWSER_EXTERNAL_OPEN_BLOCKED');
  });
});
