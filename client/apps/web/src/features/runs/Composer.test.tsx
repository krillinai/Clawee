...
describe('Composer', () => {
  ...
  it('hides reasoning controls when the selected model has no reasoning efforts', async () => {
    const user = userEvent.setup();
    const model: CodexModelResponse = {
      ...codexModels[0]!,
  ...
  it('shows and toggles only native Codex MCP connectors', async () => {
    const user = userEvent.setup();
    ...
    const onToggleConnector = vi.fn(async (_connectorId: string, enabled: boolean) => [{
      ...
      id: 'mcp:docs',
      label: 'docs',
      description: `http · ${enabled ? '已开启' : '已安装'}`,
      status: enabled ? 'enabled' as const : 'installed' as const,
      ...(enabled ? { insertText: '使用 MCP：docs ' } : {})
    }]);
    render(
      <Composer
        {...defaultProps}
    ...
    const enabledMcpList = screen.getByRole('list', { name: '已开启的 MCP' });
    const githubIcon = within(enabledMcpList).getByRole('button', {
      name: '打开连接器列表，github MCP'
    });
    expect(githubIcon).toHaveAttribute('title', 'github · 查看连接器');
    expect(githubIcon.querySelector('[data-mcp-icon="source"]'))
      .toHaveAttribute('data-tone', 'violet');
    expect(within(enabledMcpList).queryByRole('button', {
      name: '打开连接器列表，docs MCP'
    }))
    ...
    expect(screen.getByRole('switch', { name: 'github MCP' }))
      .toHaveAttribute('aria-checked', 'true');
    const docsToggle = screen.getByRole('switch', { name: 'docs MCP' });
    expect(docsToggle)
      .toHaveAttribute('aria-checked', 'false');
    expect(screen.getByRole('button', { name: '选择更多连接器' })).toBeInTheDocument();
    await user.click(docsToggle);
    expect(onToggleConnector).toHaveBeenCalledWith('mcp:docs', true);
    expect(screen.getByRole('switch', { name: 'docs MCP' }))
      .toHaveAttribute('aria-checked', 'true');
    await user.click(screen.getByRole('button', { name: '选择更多连接器' }));
    ...
    expect(screen.getByRole('menuitem', { name: /github.*已开启/ })).toBeInTheDocument();
    expect(screen.getByRole('menuitem', { name: /docs.*已开启/ })).toBeInTheDocument();
    expect(screen.queryByText('客户关系管理')).not.toBeInTheDocument();

    await user.type(screen.getByRole('searchbox', { name: '搜索连接器' }), 'docs');
    expect(screen.queryByRole('menuitem', { name: /github/ })).not.toBeInTheDocument();
    await user.click(screen.getByRole('menuitem', { name: /docs.*已开启/ }));
    expect(screen.getByRole('textbox', { name: '输入任务' })).toHaveValue('使用 MCP：docs ');
    expect(screen.getByRole('textbox', { name: '输入任务' })).toHaveAttribute('placeholder', '');
    ...
    expect(onManageConnectors).toHaveBeenCalledTimes(2);
  });

  it('keeps connector rows stable while toggling adjacent MCP servers', async () => {
    const user = userEvent.setup();
    let resolveFirstToggle!: (connectors: Array<{
      id: string;
      label: string;
      description: string;
    ...
    const onToggleConnector = vi.fn((
      ...
      if (onToggleConnector.mock.calls.length === 1) {
        return new Promise<Parameters<typeof resolveFirstToggle>[0]>(resolve => {
      ...
      return Promise.resolve([
        {
          id: 'mcp:node_repl',
          label: 'node_repl',
          description: `stdio · ${enabled ? '已开启' : '已安装'}`,
          status: enabled ? 'enabled' as const : 'installed' as const,
          ...(enabled ? { insertText: '使用 MCP：node_repl ' } : {})
        },
        {
          id: 'mcp:openaiDeveloperDocs',
          label: 'openaiDeveloperDocs',
          description: 'http · 已开启',
          status: 'enabled' as const,
          insertText: '使用 MCP：openaiDeveloperDocs '
        }
      ]);
    });
    render(
    ...
    await user.click(screen.getByRole('button', {
      name: '打开连接器列表，node_repl MCP'
    }));
    const quickList = screen.getByRole('group', { name: '连接器快捷开关' });
    const switchLabels = () => within(quickList)
      .getAllByRole('switch')
    ...
    const nodeToggle = within(quickList).getByRole('switch', {
      name: 'node_repl MCP'
    });
    const docsToggle = within(quickList).getByRole('switch', {
      name: 'openaiDeveloperDocs MCP'
    });
    await user.click(nodeToggle);

    expect(onToggleConnector).toHaveBeenCalledWith('mcp:node_repl', false);
    expect(nodeToggle).toHaveAttribute('data-updating', 'true');
    expect(nodeToggle).toHaveAttribute('aria-disabled', 'true');
    expect(docsToggle).not.toHaveAttribute('data-updating');
    expect(nodeToggle).not.toBeDisabled();
    expect(docsToggle).not.toBeDisabled();
    ...
    await waitFor(() => {
      expect(within(quickList).getByRole('switch', {
        name: 'node_repl MCP'
      })).toHaveAttribute('aria-checked', 'false');
    });
    expect(switchLabels()).toEqual([
    ...
    await user.click(within(quickList).getAllByRole('switch')[0]!);
    expect(onToggleConnector).toHaveBeenLastCalledWith('mcp:node_repl', true);
  });

  it('keeps native MCP available when local capability loading reports an error', async () => {
    const user = userEvent.setup();
    render(
      <Composer
        {...defaultProps}
    ...
    expect(screen.getByRole('alert')).toHaveTextContent('本机能力检测失败');
    await user.click(screen.getByRole('menuitem', { name: /github.*已开启/ }));
  ...
  it('supports pasted images and dropped documents through the same upload path', async () => {
    ...
    const dropped = new File(['requirements'], 'requirements.pdf', {
    ...
    expect(screen.getByTitle('requirements.pdf')).toBeInTheDocument();
  ...
  it('keeps document input available when Codex image input is unsupported', async () => {
    ...
    expect(screen.getByRole('menuitem', { name: /添加文件/ })).toBeEnabled();
    const fileInput = screen.getByLabelText('选择文件');
    await user.upload(fileInput, new File(['notes'], 'notes.md', { type: '' }));
  ...
  it('selects an external Skill while preserving the visible draft', async () => {
    const user = userEvent.setup();
    const onSkillApplied = vi.fn();
    const slashCommands = [
      {
    ...
    const { rerender } = render(
      <Composer
        ...
        onSkillApplied={onSkillApplied}
      />
    );
    const textbox = screen.getByRole('textbox', { name: '输入任务' });
    await user.type(textbox, '保留这段需求');

    rerender(
      <Composer
        {...defaultProps}
        slashCommands={slashCommands}
        skillRequest={{ id: 1, skillId: 'content-strategy' }}
        onSkillApplied={onSkillApplied}
      />
    );
    ...
    expect(await screen.findByLabelText('已选择 Skill content-strategy'))
      .toBeInTheDocument();
    expect(textbox).toHaveValue('保留这段需求');
    await waitFor(() => expect(onSkillApplied).toHaveBeenCalledWith(1));

    rerender(
      <Composer
        {...defaultProps}
        slashCommands={slashCommands}
        skillRequest={{ id: 2, skillId: 'ad-creative' }}
        onSkillApplied={onSkillApplied}
      />
    );
    expect(await screen.findByLabelText('已选择 Skill ad-creative'))
      .toBeInTheDocument();
    expect(textbox).toHaveValue('保留这段需求');
  });
