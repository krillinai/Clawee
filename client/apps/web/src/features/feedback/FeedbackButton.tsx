...
import type { FeedbackArtifact, FeedbackDraft } from '@clawee/protocol';
...
function FeedbackDialog({ client, threadId, onClose }: { client: RuntimeClient; threadId: string; onClose(): void }) {
  ...
  async function collect() {
    ...
    let native; try { native = await window.claweeDesktop?.collectFeedback?.(threadId); } catch { /* 桌面版本不可用时仍允许轻量反馈。 */ }
    await client.post(`/feedback/drafts/${created.local_feedback_id}/collect`, { native, include_diagnostics: includeDiagnostics }); await refresh(created.local_feedback_id);
  }
  ...
  const legacy = draft?.manifest && !draft.manifest.collection_scope && draft.state !== 'submitted';
  return createPortal(<dialog ref={dialog} className="feedback-dialog" onCancel={onClose}>
    ...
    {draft ? <div role="status" aria-live="polite" className={`feedback-receipt feedback-receipt--${draft.state}`}>
      ...
      <div><strong>{draft.state === 'submitted' ? '反馈提交成功' : states[draft.state]}</strong>
        ...
        {draft.centre_status || draft.state === 'submitted' ? <p>处理状态：{draft.centre_status ? centreStates[draft.centre_status] ?? '未知状态' : '已接收'}</p> : null}
    ...
    <div className="feedback-dialog-body" onPaste={event => { if (frozen) return; const files = Array.from(event.clipboardData.files).filter(f => ['image/png', 'image/jpeg', 'image/webp'].includes(f.type)); if (files.length) { event.preventDefault(); setScreenshots(old => [...old, ...files].slice(0, 5)); } }}>
      ...
      {!frozen ? <label className="feedback-check"><input type="checkbox" checked={includeDiagnostics} onChange={e => setIncludeDiagnostics(e.target.checked)} />附加诊断信息（最近 10 分钟，最多 200 条）</label> : null}
      {draft ? <>{draft.public_resolution_summary ? <p>{draft.public_resolution_summary}</p> : null}<p>{draft.state === 'submitted' ? '本地回执' : '本地材料'}保留至 {new Date(draft.expires_at).toLocaleString()}</p>{draft.materials_expires_at ? <p>诊断副本和截图保留至 {new Date(draft.materials_expires_at).toLocaleString()}</p> : null}{legacy ? <p role="alert">此草稿包含旧版完整采集材料，已停止上传。请新建轻量反馈。</p> : null}{draft.manifest ? <>
        {draft.manifest.collection_scope ? <p>发送范围：问题说明、{draft.screenshots.length} 张截图、版本信息和{draft.manifest.collection_scope === 'diagnostics' ? '附加诊断摘要' : '基础错误摘要'}。不包含会话正文、工具输入输出、附件索引或原始日志。</p> : null}
        {draft.preview ? <details className="feedback-preview"><summary>查看发送内容</summary><pre>{JSON.stringify(draft.preview, null, 2)}</pre></details> : null}
        {frozen && screenshots.length ? <div className="feedback-screenshots">{screenshots.map((file, index) => <ScreenshotEditor key={`${index}:${file.name}`} file={file} readOnly onChange={() => {}} onRemove={() => {}} />)}</div> : null}
        {!screenshots.length && draft.manifest.artifacts.some(a => a.kind === 'screenshot') ? <div className="feedback-screenshots">{draft.manifest.artifacts.filter(a => a.kind === 'screenshot').map(artifact => <ScreenshotPreview key={artifact.artifact_id} client={client} draftId={draft.local_feedback_id} artifact={artifact} />)}</div> : null}
        <details><summary>材料明细</summary><ul>{draft.manifest.artifacts.map(a => <li key={a.artifact_id}>{a.name} · {a.record_count} 条 · {(a.size_bytes / 1024).toFixed(1)} KiB</li>)}</ul>{draft.manifest.missing_items.length ? <><h3>未能采集的材料</h3><ul>{draft.manifest.missing_items.map(item => <li key={item}>{item}</li>)}</ul></> : null}</details>
      </> : null}{draft.error_code ? <p role="alert">{draft.error_code}；材料保留在本机。</p> : null}</> : null}
      {draft?.state === 'awaiting_consent' && draft.manifest && !legacy ? <><label className="feedback-check"><input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)} />同意将上述问题说明、主动添加的截图及诊断摘要发送给 Clawee 软件维护方 gateway.clawee.work。</label>{draft.manifest.completeness === 'partial' ? <label className="feedback-check"><input type="checkbox" checked={partial} onChange={e => setPartial(e.target.checked)} />已查看缺失项，同意发送部分资料</label> : null}</> : null}
      ...
      {draft?.state === 'awaiting_consent' && draft.manifest && !legacy ? <button type="button" disabled={busy || !confirmed || (draft.manifest.completeness === 'partial' && !partial)} onClick={() => void run(async () => { await client.post(`/feedback/drafts/${draft.local_feedback_id}/send`, { confirmed, manifest_sha256: draft.manifest_sha256, accept_partial: partial }); await refresh(draft.local_feedback_id); })}>确认发送</button> : null}
      {draft?.state === 'failed' && draft.error_code !== 'FEEDBACK_QUOTA_EXCEEDED' && !legacy ? <button type="button" disabled={busy} onClick={() => void run(async () => { await client.post(`/feedback/drafts/${draft.local_feedback_id}/${draft.consent_at ? 'retry' : 'collect'}`, {}); await refresh(draft.local_feedback_id); })}>重试</button> : null}
      ...
      {draft ? <button type="button" disabled={busy || ['collecting', 'queued', 'uploading'].includes(draft.state)} onClick={() => { localStorage.removeItem(receiptKey); setDraft(undefined); setConfirmed(false); setPartial(false); setIncludeDiagnostics(false); }}>新建反馈</button> : null}
...
function ScreenshotPreview({ client, draftId, artifact }: { client: RuntimeClient; draftId: string; artifact: FeedbackArtifact }) {
  const [url, setUrl] = useState('');
  const [failed, setFailed] = useState(false);
  useEffect(() => {
    let active = true; let objectUrl = '';
    void client.rawGet(`/feedback/drafts/${draftId}/artifacts/${artifact.artifact_id}`).then(response => response.blob()).then(blob => {
      if (!active) return; objectUrl = URL.createObjectURL(new Blob([blob], { type: artifact.content_type })); setUrl(objectUrl);
    }).catch(() => { if (active) setFailed(true); });
    return () => { active = false; if (objectUrl) URL.revokeObjectURL(objectUrl); };
  }, [client, draftId, artifact.artifact_id, artifact.content_type]);
  return failed ? <p role="alert">截图预览不可用，请新建反馈重新添加截图。</p> : url ? <img src={url} alt="发送截图预览" onError={() => setFailed(true)} /> : <LoaderCircle size={18} aria-label="正在读取截图" />;
}
function ScreenshotEditor({ file, onChange, onRemove, readOnly = false }: { file: File; onChange(file: File): void; onRemove(): void; readOnly?: boolean }) {
  ...
  return <div><canvas ref={canvas} aria-label={readOnly ? '发送截图预览' : '截图预览与遮盖'} title={readOnly ? '发送截图预览' : '截图预览与遮盖'} onPointerDown={e => { if (readOnly) return; const c = e.currentTarget; const rect = c.getBoundingClientRect(); start.current = { x: (e.clientX - rect.left) * c.width / rect.width, y: (e.clientY - rect.top) * c.height / rect.height }; c.setPointerCapture(e.pointerId); }} onPointerUp={e => { if (readOnly || !start.current) return; const c = e.currentTarget; const rect = c.getBoundingClientRect(); const x = (e.clientX - rect.left) * c.width / rect.width; const y = (e.clientY - rect.top) * c.height / rect.height; const ctx = c.getContext('2d'); if (ctx) { ctx.fillStyle = '#000000'; ctx.fillRect(Math.min(x, start.current.x), Math.min(y, start.current.y), Math.abs(x - start.current.x), Math.abs(y - start.current.y)); c.toBlob(blob => { if (blob) onChange(new File([blob], file.name, { type: 'image/png' })); }, 'image/png'); } start.current = undefined; }} />{!readOnly ? <button type="button" title="移除截图" aria-label="移除截图" onClick={onRemove}><Trash2 size={16} /></button> : null}</div>;
}
