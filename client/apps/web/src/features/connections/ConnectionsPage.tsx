...
export type ConnectionsPageProps = {
  ...
  mcpCatalog?: EnterpriseMcpCatalogResponse;
  mcpCapabilities?: McpCapabilities;
  onMcpDataChange?(data: CodexMcpListResponse): void;
  onMcpCatalogChange?(data: EnterpriseMcpCatalogResponse | undefined): void;
  onOpenAccount(): void;
...
export function ConnectionsPage(props: ConnectionsPageProps) {
  ...
  const [catalog, setCatalog] = useState<EnterpriseMcpCatalogResponse | undefined>(
    props.mcpCatalog
  );
  const catalogRef = useRef(props.mcpCatalog);
  ...
  useEffect(() => {
    catalogRef.current = props.mcpCatalog;
    setCatalog(props.mcpCatalog);
  }, [props.mcpCatalog]);
  ...
  useEffect(() => {
    ...
    if (props.session.status !== 'signed_in') updateCatalog(undefined);
    let canceled = false;
    void loadConnections(
      props.session.status === 'signed_in',
      canceledRef => canceledRef || canceled
    );
    return () => {
      canceled = true;
    };
  ...
  function updateCatalog(next: EnterpriseMcpCatalogResponse | undefined) {
    catalogRef.current = next;
    setCatalog(next);
    props.onMcpCatalogChange?.(next);
  }
  ...
  async function loadConnections(
    ...
    const enterpriseResult = await Promise.resolve(enterpriseWork)
      .then(
        value => ({ status: 'fulfilled' as const, value }),
        reason => ({ status: 'rejected' as const, reason })
      );
    const nativeResult = await Promise.resolve(props.mcpService.listServers())
      .then(
        value => ({ status: 'fulfilled' as const, value }),
        reason => ({ status: 'rejected' as const, reason })
      );
    ...
    if (enterpriseResult.status === 'fulfilled') {
      updateCatalog(enterpriseResult.value);
    } else {
  ...
  const connections = useMemo(
    ...
    [catalog?.upstreams, nativeData?.presets, nativeData?.servers]
  );
  ...
  async function installEnterprise(item: UnifiedConnection) {
    ...
    setLoadError(undefined);
    try {
      updateCatalog(await props.service.updateMcpPreference(
  ...
  async function installPreset(item: UnifiedConnection) {
    if (
      props.mcpService === null
    ...
    try {
      await props.mcpService.addServer({
        ...
        transport: item.preset.transport,
        command: item.preset.command,
        args: item.preset.args,
        ...(nativeData?.requiresWriteConfirmation === true
  ...
  function installConnection(item: UnifiedConnection) {
    if (item.preset !== undefined) {
      void installPreset(item);
      return;
    }
    void installEnterprise(item);
  }
    ...
    <main className="connections-page">
      <div className="connections-page__inner">
        ...
        {loading && nativeData === undefined ? (
          <div className="connections-empty connections-empty--loading">
          ...
          <section className="connections-grid" aria-label="连接器目录">
            {filtered.map(item => (
              <ConnectionCard
                ...
                capabilities={props.mcpCapabilities}
                onInstall={() => installConnection(item)}
        ...
        <span>
          {props.item.upstream !== undefined
            ...
            : props.item.preset !== undefined
              ? `${props.item.preset.provider} 本机预设`
              : 'Codex 原生配置'}
        </span>
      </div>
...
function mergeConnections(
  ...
  const matchedPresetNames = new Set<string>();
  const installed = servers.map(server => {
    ...
    const namedPreset = presets.find(candidate => candidate.name === server.name);
    const preset = namedPreset !== undefined
      && matchesPresetConfiguration(server, namedPreset)
        ? namedPreset
        : undefined;
    if (upstream !== undefined) matchedUpstreamIds.add(upstream.upstreamId);
    if (namedPreset !== undefined) matchedPresetNames.add(namedPreset.name);
    return {
      key: `native:${server.name}`,
      name: upstream?.name ?? preset?.displayName ?? server.name,
      subtitle: upstream !== undefined
        ? `${upstream.domain || upstream.namespace} · ${server.name}`
        : preset !== undefined
          ? `${preset.provider} · ${preset.platform}`
          : server.name,
  ...
  const availablePresets = presets
    .filter(preset => !matchedPresetNames.has(preset.name))
    .map(preset => ({
      key: `preset:${preset.name}`,
      name: preset.displayName,
      subtitle: `${preset.provider} · ${preset.platform}`,
      endpoint: mcpPresetEndpoint(preset),
      installed: false,
      enabled: false,
      preset
    }));
  ...
  return [...installed, ...availablePresets, ...availableUpstreams].sort((left, right) =>
...
function mcpPresetEndpoint(preset: CodexMcpPresetResponse): string {
  return [preset.command, ...preset.args].join(' ');
}

function matchesPresetConfiguration(
  server: CodexMcpServerResponse,
  preset: CodexMcpPresetResponse
): boolean {
  return (
    server.transport === preset.transport
    && server.command === preset.command
    && (server.args ?? []).length === preset.args.length
    && (server.args ?? []).every((arg, index) => arg === preset.args[index])
  );
}
...
function formatConnectionError(
  ...
  if (typeof error === 'object' && error !== null && 'code' in error) {
    ...
    if (code === 'ENTERPRISE_MCP_UPSTREAM_NOT_FOUND') {
      return '企业 MCP 目录已更新，请刷新连接器后重试';
    }
    if (code === 'ENTERPRISE_MCP_RUNTIME_UNAVAILABLE') {
      return '企业 MCP 运行配置暂时不可用';
    }
