...
describe('FileWorkspaceView', () => {
  ...
  it('同一项目切换到没有成果文件的会话时保持空白', async () => {
    ...
    expect(service.getMeta).not.toHaveBeenCalledWith(emptyThread.id, 'A.md');
  });

  it('切换会话后忽略上一会话尚未完成的文件预览', async () => {
    const firstThread = createThread({ id: 'thread-slow-file' });
    const emptyThread = createThread({ id: 'thread-empty' });
    let resolveMeta: ((meta: WorkspaceFileMeta) => void) | undefined;
    const pendingMeta = new Promise<WorkspaceFileMeta>((resolve) => {
      resolveMeta = resolve;
    });
    const service = createService({
      directories: {
        '': createDirectory({ nodes: [fileNode('A.md', 'markdown')] })
      },
      contents: { 'A.md': '# A' }
    });
    service.getMeta.mockImplementation(async (threadId: string, path: string) => {
      if (threadId === firstThread.id && path === 'A.md') return await pendingMeta;
    ...
    await waitFor(() => expect(service.getMeta).toHaveBeenCalledWith(firstThread.id, 'A.md'));
    ...
    await waitFor(() => expect(service.listDirectory).toHaveBeenCalledWith(emptyThread.id, ''));
    resolveMeta?.(createMeta({ path: 'A.md', name: 'A.md' }));

    expect(await screen.findByText('选择一个文件')).toBeInTheDocument();
    expect(service.openText).not.toHaveBeenCalledWith(firstThread.id, 'A.md');
    expect(screen.queryByRole('heading', { name: 'A' })).not.toBeInTheDocument();
  });

  it('用户手动选择文件后忽略尚未完成的自动预览', async () => {
    const thread = createThread();
    let resolveAutoMeta: ((meta: WorkspaceFileMeta) => void) | undefined;
    const pendingAutoMeta = new Promise<WorkspaceFileMeta>((resolve) => {
      resolveAutoMeta = resolve;
    });
    const service = createService({
      directories: {
        '': createDirectory({
          nodes: [fileNode('auto.md', 'markdown'), fileNode('manual.md', 'markdown')]
        })
      },
      contents: {
        'auto.md': '# Auto',
    ...
    service.getMeta.mockImplementation(async (_threadId: string, path: string) => (
      path === 'auto.md'
        ? await pendingAutoMeta
        : createMeta({ path, name: path })
    ));

    const view = render(
      <FileWorkspaceView
        selectedThread={thread}
        conversationPaths={['auto.md']}
        workspaceFileService={service}
        onClose={vi.fn()}
      />
    );
    await waitFor(() => expect(service.getMeta).toHaveBeenCalledWith(thread.id, 'auto.md'));

    view.rerender(
      <FileWorkspaceView
        selectedThread={thread}
        selectedPath="manual.md"
        conversationPaths={['auto.md']}
        workspaceFileService={service}
        onClose={vi.fn()}
      />
    ...
    resolveAutoMeta?.(createMeta({ path: 'auto.md', name: 'auto.md' }));

    await waitFor(() => {
      expect(screen.getByRole('heading', { name: 'Manual' })).toBeInTheDocument();
    });
    expect(service.openText).not.toHaveBeenCalledWith(thread.id, 'auto.md');
  });

  it('只自动打开当前会话匹配的文件', async () => {
    const thread = createThread();
    const service = createService({
      directories: {
        '': createDirectory({
  ...
  it('当前会话没有匹配文件时不打开项目推荐文件', async () => {
    ...
    const service = createService({
      directories: {
        '': createDirectory({
          ...
          nodes: [
            directoryNode('.gstack'),
            fileNode('unrelated.html', 'html')
          ]
        })
      }
    });
    ...
    expect(service.openText).not.toHaveBeenCalled();

    await expandFileTree(user);
    expect(screen.getByText('当前工作区暂无可显示文件')).toBeInTheDocument();
    expect(screen.queryByRole('treeitem', { name: '.gstack' })).not.toBeInTheDocument();
    expect(screen.queryByRole('treeitem', { name: 'unrelated.html' }))
      .not.toBeInTheDocument();
  });

  it('目录树只列出当前会话文件及其父目录', async () => {
    const user = userEvent.setup();
    const thread = createThread();
    const service = createService({
      directories: {
        ...
        reports: createDirectory({
          path: 'reports',
          nodes: [
            fileNode('reports/current.html', 'html', 1),
            fileNode('reports/old.html', 'html', 1)
          ]
        })
      },
      metas: {
        'reports/current.html': createMeta({
          path: 'reports/current.html',
          name: 'current.html',
          kind: 'html',
          mime: 'text/html'
        })
      ...
      contents: {
        'reports/current.html': '<h1>Current</h1>'
      }
    });
    ...
    render(
      <FileWorkspaceView
        selectedThread={thread}
        conversationPaths={['reports/current.html']}
        workspaceFileService={service}
        onClose={vi.fn()}
      />
    );

    expect(await screen.findByTitle('current.html HTML 预览')).toBeInTheDocument();
    await expandFileTree(user);
    expect(screen.getByRole('treeitem', { name: 'reports' })).toBeInTheDocument();
    expect(screen.queryByRole('treeitem', { name: '.gstack' })).not.toBeInTheDocument();
    expect(screen.queryByRole('treeitem', { name: 'unrelated.html' }))
      .not.toBeInTheDocument();

    await user.click(screen.getByRole('treeitem', { name: 'reports' }));
    await waitFor(() => expect(service.listDirectory).toHaveBeenCalledWith(
      thread.id,
      'reports'
    ));
    expect(await screen.findByRole('treeitem', { name: 'current.html' }))
      .toBeInTheDocument();
    expect(screen.queryByRole('treeitem', { name: 'old.html' })).not.toBeInTheDocument();
  });

  it('跳过当前会话中不可预览或已失效的文件，并打开下一个可预览成果', async () => {
    const thread = createThread();
    const service = createService({
      directories: {
        '': createDirectory({
          nodes: [fileNode('raw.bin', 'binary'), fileNode('report.html', 'html')]
        })
      },
      metas: {
        'raw.bin': createMeta({
      ...
      contents: { 'report.html': '<h1>Report</h1>' }
    });
    service.getMeta.mockRejectedValueOnce(new Error('missing'));

    render(
      <FileWorkspaceView
        selectedThread={thread}
        conversationPaths={['missing.pdf', 'raw.bin', 'report.html']}
        workspaceFileService={service}
        onClose={vi.fn()}
      />
    );

    expect(await screen.findByTitle('report.html HTML 预览')).toBeInTheDocument();
    expect(service.getMeta).toHaveBeenCalledWith(thread.id, 'missing.pdf');
    expect(service.getMeta).toHaveBeenCalledWith(thread.id, 'raw.bin');
    expect(service.getMeta).toHaveBeenCalledWith(thread.id, 'report.html');
  });
  ...
  it('点击目录时调用 listDirectory 并合并节点', async () => {
    ...
    render(
      <FileWorkspaceView
        selectedThread={thread}
        conversationPaths={['docs/guide.md']}
        workspaceFileService={service}
        onClose={vi.fn()}
      />
    );
  ...
  it('点击图片和 PDF 时调用 getMeta 和 openBlob', async () => {
    ...
    render(
      <FileWorkspaceView
        selectedThread={thread}
        conversationPaths={['preview.png', 'spec.pdf']}
        workspaceFileService={service}
        onClose={vi.fn()}
      />
    );
  ...
  it('主工作区 DOM 顺序为编辑区在前、文件树在后', async () => {
    ...
    const { container } = render(
      <FileWorkspaceView selectedThread={thread} conversationPaths={['README.md']} workspaceFileService={service} onClose={vi.fn()} />
    );
  ...
  it('使用单层文件工具栏，避免重复的路径栏和编辑器标题栏', async () => {
    ...
    const { container } = render(
      <FileWorkspaceView selectedThread={thread} conversationPaths={['scripts/notes.md']} workspaceFileService={service} onClose={vi.fn()} />
    );
  ...
  it('在顶部工具栏预览 HTML 时渲染实际页面', async () => {
    ...
    render(<FileWorkspaceView selectedThread={thread} conversationPaths={['business-cover.html']} workspaceFileService={service} onClose={vi.fn()} />);
  ...
  it('可以收起和展开右侧目录树，编辑区保持可用', async () => {
    ...
    render(<FileWorkspaceView selectedThread={thread} conversationPaths={['README.md']} workspaceFileService={service} onClose={vi.fn()} />);
  ...
  it('切换 blob 文件或卸载时会 revokeBlob', async () => {
    ...
    const { unmount } = render(
      <FileWorkspaceView
        selectedThread={thread}
        conversationPaths={['preview.png', 'notes.txt']}
        workspaceFileService={service}
        onClose={vi.fn()}
      />
    );
  ...
  it('编辑后切换文件时，取消 confirm 不切换，确认后才切换', async () => {
    ...
    render(
      <FileWorkspaceView
        selectedThread={thread}
        conversationPaths={['notes.txt', 'guide.md']}
        workspaceFileService={service}
        onClose={vi.fn()}
      />
    );
  ...
  it('409 冲突时重新加载会重新打开当前文件，覆盖保存会带 overwriteConflict，取消会关闭提示', async () => {
    ...
    render(<FileWorkspaceView selectedThread={thread} conversationPaths={['notes.txt']} workspaceFileService={service} onClose={vi.fn()} />);
