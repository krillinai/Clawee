...
describe('HtmlPreview', () => {
  ...
  it('removes user scripts and navigational content while allowing only the preview runtime', async () => {
    ...
    expect(srcDoc).toContain('scrollbar-color: rgb(127 127 127 / 28%) transparent');
    expect(srcDoc).toContain('*::-webkit-scrollbar');
    expect(srcDoc).toContain('width: 5px');
    expect(srcDoc).toContain('background: transparent');
    expect(srcDoc).toContain('*::-webkit-scrollbar-thumb:hover');
