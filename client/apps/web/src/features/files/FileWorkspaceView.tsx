...
export function FileWorkspaceView(props: FileWorkspaceViewProps) {
  ...
  const conversationPathsRef = useRef(props.conversationPaths ?? []);
  conversationPathsRef.current = props.conversationPaths ?? [];
  const threadIdRef = useRef(props.selectedThread?.id);
  threadIdRef.current = props.selectedThread?.id;
  ...
  const visibleNodes = useMemo(() => filterDirectoryNodesByPaths(
    nodes,
    [
      ...(props.conversationPaths ?? []),
      ...(props.selectedPath === undefined ? [] : [props.selectedPath])
    ]
  ), [nodes, props.conversationPaths, props.selectedPath]);
    ...
    void service
      ...
      .then(async (directory) => {
        ...
        if (requestedPath && requestedPath.length > 0) {
          ...
          await openFirstConversationPreview(
            currentThread,
            conversationPathsRef.current,
            directory.nodes
          );
        }
      })
  ...
  }, [thread?.id, thread?.sandbox, thread?.status, service]);
  ...
  useEffect(() => {
    ...
    void openFirstConversationPreview(thread, props.conversationPaths ?? [], nodes);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.conversationPaths]);
  ...
  async function openFilePath(
    path: string,
    options?: { skipDirtyConfirm?: boolean; knownMeta?: WorkspaceFileMeta }
  ) {
    if (!thread || !service) {
      return;
    }
  ...
  async function openFirstConversationPreview(
    currentThread: ThreadResponse,
    paths: string[],
    directoryNodes: WorkspaceDirectoryResponse['nodes']
  ) {
    openRequestIdRef.current += 1;
    const scanRequestId = openRequestIdRef.current;

    for (const path of paths) {
      if (
        !mountedRef.current
        || currentThread.id !== threadIdRef.current
        || scanRequestId !== openRequestIdRef.current
      ) return;
      const resolvedPath = resolveDirectoryFilePath(path, directoryNodes);
      try {
        const candidateMeta = await service?.getMeta(currentThread.id, resolvedPath);
        if (
          candidateMeta?.previewable !== true
          || !mountedRef.current
          || currentThread.id !== threadIdRef.current
          || scanRequestId !== openRequestIdRef.current
        ) {
        ...
        await openFilePath(resolvedPath, {
          skipDirtyConfirm: true,
          knownMeta: candidateMeta
        });
        return;
      } catch {
        // Stale or deleted conversation artifacts are skipped without selecting project files.
      }
    }
  }
  ...
  async function handleToggleDirectory(path: string) {
    ...
    if (expandedPaths.includes(path) && loadedPathsRef.current.has(path)) {
