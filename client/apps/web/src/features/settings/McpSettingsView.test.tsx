...
describe('McpSettingsView', () => {
  it('lists servers without rendering secret values and adds a stdio server', async () => {
    ...
    await user.type(screen.getByLabelText('启动参数'), 'server.js');
  ...
  it('adds a remote MCP without exposing local command fields', async () => {
    const user = userEvent.setup();
    const addServer = vi.fn(async (
      _input: AddCodexMcpRequest
    ): Promise<CodexMcpAddResponse> => ({
      operation: operation('add')
    }));
    render(
      <McpSettingsView
    ...
    await user.click(screen.getByRole('button', { name: '新增 MCP' }));
    await user.click(screen.getByRole('button', { name: '远程地址' }));

    expect(screen.queryByLabelText('启动命令')).not.toBeInTheDocument();
    expect(screen.getByLabelText('远程协议')).toHaveValue('http');
    await user.type(screen.getByLabelText('名称'), 'notion');
    await user.type(
      screen.getByLabelText('远程 MCP 地址'),
      'https://mcp.example.com/notion'
    );
    await user.click(screen.getByRole('button', { name: '保存 MCP' }));
