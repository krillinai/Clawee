...
describe('App', () => {
  it('shows the enterprise model configuration gate when login is not required', async () => {
    ...
    const runtimeFetch = async (input: RequestInfo | URL) => {
      ...
      if (url.endsWith('/runtime/model-service')) {
        return jsonResponse({
    ...
    expect(document.querySelector('.clawee-shell')).not.toBeInTheDocument();
    expect(requests.some(url => url.endsWith('/codex/status'))).toBe(false);
  });

  it.each(['browser', 'desktop'] as const)(
    '%s host shows enterprise login before resolving model configuration',
    async hostKind => {
      ...
      const runtimeFetch = async (input: RequestInfo | URL) => {
        ...
        if (url.endsWith('/runtime/model-service')) {
          return jsonResponse({ status: 'resolving' });
        }
      ...
      render(
        <ProductionApp hostBridge={hostBridge} runtimeFetch={runtimeFetch} />
      );
  ...
  it('does not expose the manual form when platform model configuration is unavailable', async () => {
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
      token: 'runtime-token'
    ...
    const runtimeFetch = async (input: RequestInfo | URL) => {
      ...
      if (url.endsWith('/runtime/model-service/retry')) {
        retryRequests += 1;
  ...
  it('returns to enterprise login when the session expires during model resolution', async () => {
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
      token: 'runtime-token'
    ...
    const runtimeFetch = async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url.endsWith('/healthz')) {
        return jsonResponse({ ok: true, runtimeState: 'configuration_required' });
      }
      if (url.endsWith('/runtime/model-service')) {
        return jsonResponse({ status: 'resolving' });
      }
      if (url.endsWith('/enterprise/session')) {
        ...
        return jsonResponse(createEnterpriseSessionResponse(
          sessionReads === 1
            ? {}
            : { status: 'signed_out', account: undefined }
        ));
      }
      throw new Error(`Unexpected request ${url}`);
    };
  ...
  it('maps valid skills and configured MCP servers into Composer commands', () => {
    const commands = buildComposerSlashCommands(createSkillListResponse([
    ...
    expect(commands).not.toEqual(expect.arrayContaining([
      expect.objectContaining({ id: 'mcp:docs' })
    ]));
  });

  it('maps configured Codex MCP servers into the Composer connector list', () => {
    expect(buildComposerMcpConnectors({
      codexHome: '/tmp/codex',
      codexHomeMode: 'global',
      requiresWriteConfirmation: false,
      servers: [{
        ...
        codexHome: '/tmp/codex',
        codexHomeMode: 'global',
        diagnostics: ['invalid configuration']
      }],
      presets: [],
      diagnostics: []
    })).toEqual([{
      id: 'mcp:github',
      label: 'github',
      description: 'http · 已开启',
      status: 'enabled',
      insertText: '使用 MCP：github '
    }, {
      id: 'mcp:docs',
      label: 'docs',
      description: 'stdio · 已安装',
      status: 'installed'
    }]);
  });

  it('uses enterprise display names while preserving the semantic MCP name', () => {
    const mcp = {
      codexHome: '/tmp/codex',
    ...
    expect(buildComposerMcpConnectors(mcp, catalog)).toEqual([{
      id: 'mcp:enterprise_crm',
      label: '客户关系管理',
      description: 'enterprise_crm · http · 已开启',
      status: 'enabled',
      insertText: '使用连接器「客户关系管理」（MCP：enterprise_crm） '
    }]);
    expect(buildComposerSlashCommands(undefined, mcp, catalog)).toEqual([{
      id: 'mcp:enterprise_crm',
      category: 'mcp',
      label: '客户关系管理',
      description: 'enterprise_crm · http · 已开启',
  ...
  it.each(['browser', 'desktop'] as const)(
    '%s restores billing, activity list, and detail views from browser history routes',
    async hostKind => {
    const user = userEvent.setup();
    const hostBridge = createHostBridge();
    hostBridge.kind = hostKind;
    const openExternal = vi.fn(async () => undefined);
    hostBridge.openExternal = openExternal;
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
      token: 'runtime-token'
    });
    const runtimeFetch = createKnowledgeRuntimeFetch(url => {
      ...
      if (url.endsWith('/enterprise/activity/capability')) {
        return jsonResponse({
          allowed: true,
          refreshedAt: '2026-08-18T08:00:00.000Z'
        });
      }
      if (url.includes('/enterprise/activity/statistics?')) {
        const range = new URL(url).searchParams.get('range') as EnterpriseActivityRange;
        return jsonResponse(createActivityStatisticsResponse(range));
      }
      if (url.includes('/enterprise/activity/detail?')) {
        const parsed = new URL(url);
        return jsonResponse(createActivityDetailResponse({
          collectorId: parsed.searchParams.get('collectorId') ?? '',
          agentId: parsed.searchParams.get('agentId') ?? ''
        }));
      }
      if (url.includes('/enterprise/billing/overview?')) {
        return jsonResponse({
          balanceCny: 100,
          generatedAt: '2026-08-28T10:00:00+08:00'
        });
      }
      if (url.includes('/enterprise/billing/recharge-orders?')) {
        return jsonResponse({
          items: [{
            orderNo: 'PAY20260828100000abcdef123456789',
            amountCents: 10_000,
            currency: 'CNY',
            channel: 'alipay',
            ...
            paidAt: '2026-08-28T10:01:00+08:00'
          }],
          page: 1,
          total: 1
        });
      }
      if (url.endsWith('/enterprise/billing/recharge-session')) {
        return jsonResponse({
    ...
    expect(screen.getByText('已支付，到账异常')).toBeInTheDocument();
    expect(window.location.hash).toBe('#/activity/recharge-records?range=7d');
  ...
  it('keeps billing disabled in enterprise-managed mode on activity routes', async () => {
    const user = userEvent.setup();
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
    ...
    const runtimeFetch = createKnowledgeRuntimeFetch(url => {
      ...
      if (url.endsWith('/enterprise/activity/capability')) {
        return jsonResponse({ allowed: true, refreshedAt: new Date(0).toISOString() });
      }
      if (url.includes('/enterprise/activity/statistics?')) {
        return jsonResponse(createActivityStatisticsResponse('7d'));
      }
      if (url.includes('/enterprise/billing/')) {
        billingRequests += 1;
        return jsonResponse({}, { status: 500 });
      }
      return undefined;
    });
    window.location.hash = '#/activity/recharge-records?range=7d';

    render(<App hostBridge={hostBridge} runtimeFetch={runtimeFetch} />);

    expect(await screen.findByText(
      '当前部署使用企业自有模型服务，平台不管理该模型账单'
    )).toBeInTheDocument();
    expect(billingRequests).toBe(0);
    await user.click(screen.getByRole('button', { name: '返回 Agent动态' }));
    expect(await screen.findByRole('heading', { name: 'Agent动态' })).toBeInTheDocument();
    expect(screen.queryByRole('heading', { name: '账户额度' })).not.toBeInTheDocument();
    expect(billingRequests).toBe(0);
  });
  ...
  it('opens the dashboard from the homepage data analysis shortcut', async () => {
    const user = userEvent.setup();
    render(<App />);

    await user.click(await screen.findByRole('button', { name: '数据分析' }));
  ...
  it.each(['browser', 'desktop'] as const)(
    '%s host loads an installed homepage Skill into the current composer',
    async (hostKind) => {
      const user = userEvent.setup();
      const hostBridge = createHostBridge();
      hostBridge.kind = hostKind;
      ...
      const runtimeFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.endsWith('/codex/skills')) {
          return jsonResponse(createSkillListResponse([
            createSkillResponse({ id: 'content-strategy', name: 'content-strategy' })
          ]));
        }
      ...
      await screen.findByRole('button', { name: '选择项目 content-design' });
      await user.type(screen.getByRole('textbox', { name: '输入任务' }), '保留这段需求');
      await user.click(screen.getByRole('button', { name: '脚本选题生成' }));

      expect(await screen.findByLabelText('已选择 Skill content-strategy'))
        .toBeInTheDocument();
  ...
  it('installs a missing enterprise Skill before loading it into the composer', async () => {
    const user = userEvent.setup();
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
    ...
    let enterpriseSkillRequests = 0;
    const enterpriseSkill = createEnterpriseSkillResponse({
      skillId: 'ad-creative',
      name: 'ad-creative',
    ...
    const installedEnterpriseSkill = createEnterpriseSkillResponse({
      skillId: 'ad-creative',
      name: 'ad-creative',
      status: 'installed',
      integrity: 'verified',
    ...
    const runtimeFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/enterprise/skills')) {
        enterpriseSkillRequests += 1;
        return jsonResponse(createEnterpriseSkillListResponse([
          enterpriseSkillRequests === 1 ? enterpriseSkill : installedEnterpriseSkill
        ]));
      }
      if (
        url.endsWith('/enterprise/skills/ad-creative/install')
        && init?.method === 'POST'
      ) {
        return jsonResponse({
          skill: installedEnterpriseSkill,
          localSkill: createSkillResponse({ id: 'ad-creative', name: 'ad-creative' }),
          operation: {}
        });
      }
      if (url.endsWith('/codex/skills')) {
        ...
        return jsonResponse(createSkillListResponse(
          localSkillRequests === 1
            ? []
            : [createSkillResponse({ id: 'ad-creative', name: 'ad-creative' })]
        ));
      }
      if (url.endsWith('/codex/mcp')) return jsonResponse(createMcpListResponse());
      throw new Error(`Unexpected request ${url}`);
    ...
    expect(await screen.findByRole('heading', {
      name: /^(上午|下午|晚上)好，Member$/
    })).toBeInTheDocument();
    await user.click(screen.getByRole('button', { name: '广告素材审核' }));

    expect(await screen.findByLabelText('已选择 Skill ad-creative')).toBeInTheDocument();
    expect(findPostCalls(fetchCalls, '/enterprise/skills/ad-creative/install')).toHaveLength(1);
    expect(enterpriseSkillRequests).toBe(2);
    expect(localSkillRequests).toBe(2);
  });

  it('shows an error and does not fake Skill selection when installation fails', async () => {
    const user = userEvent.setup();
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
    ...
    const runtimeFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (
        url.endsWith('/enterprise/skills/marketing-video/install')
        && init?.method === 'POST'
      ) {
        return jsonResponse({
          error: {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('企业Skills暂时不可用');
    expect(screen.queryByLabelText('已选择 Skill marketing-video')).not.toBeInTheDocument();
  });
  ...
  it.each(['browser', 'desktop'] as const)(
    '%s host opens a normal conversation from the knowledge page',
    async hostKind => {
      const user = userEvent.setup();
      window.location.hash = '#/knowledge';
      const hostBridge: HostBridge = { ...createHostBridge(), kind: hostKind };
      ...
      await user.click(await screen.findByRole('button', { name: '在对话中使用' }));

      await waitFor(() => expect(window.location.hash).toBe('#/'));
      expect(fetchCalls.some(call => (
        call.url.endsWith('/enterprise/knowledge-conversations')
        && call.init?.method === 'POST'
      ))).toBe(false);
      expect(document.querySelector('.conversation-page'))
        .toHaveClass('is-empty');
      expect(document.querySelector('.conversation-page'))
        .not.toHaveClass('has-header');
      expect(screen.getByRole('textbox', { name: '输入任务' })).toBeEnabled();
      expect(screen.queryByRole('complementary', { name: '知识库文件目录' }))
        .not.toBeInTheDocument();
  ...
  it('shows cached history immediately while refreshing a revisited conversation', async () => {
    const user = userEvent.setup();
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
    ...
    const refreshedHistoryA = createDeferred<Response>();
    ...
    const runtimeFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/codex/status')) return jsonResponse(createCodexStatusResponse());
      if (url.endsWith('/threads?status=active&limit=50')) {
        return jsonResponse({ threads: [threadA, threadB] });
      }
      if (url.endsWith('/threads/thread-cache-a/history?limit=50')) {
        historyARequests += 1;
        if (historyARequests > 1) return refreshedHistoryA.promise;
        return jsonResponse({
          threadId: threadA.id,
          codexThreadId: null,
          items: [{
      ...
      if (url.endsWith('/threads/thread-cache-b/history?limit=50')) {
        return jsonResponse({
          threadId: threadB.id,
          codexThreadId: null,
          items: [{
      ...
      if (url.includes('/runs?limit=50')) return jsonResponse({ runs: [] });
      throw new Error(`Unexpected request ${url}`);
    };
    ...
    await user.click(await screen.findByRole('button', { name: /缓存会话 A/ }));
    expect(await screen.findByText('会话 A 的缓存消息')).toBeInTheDocument();
    await user.click(screen.getByRole('button', { name: /缓存会话 B/ }));
    expect(await screen.findByText('会话 B 的消息')).toBeInTheDocument();

    await user.click(screen.getByRole('button', { name: /缓存会话 A/ }));

    expect(screen.getByText('会话 A 的缓存消息')).toBeInTheDocument();
    expect(screen.queryByRole('status', { name: '正在加载会话历史' }))
      .not.toBeInTheDocument();
    expect(screen.getByRole('textbox', { name: '输入任务' })).toBeEnabled();
    expect(historyARequests).toBe(2);

    refreshedHistoryA.resolve(jsonResponse({
      threadId: threadA.id,
      codexThreadId: null,
      items: [{
        id: 'history-cache-a',
    ...
    expect(await screen.findByText('会话 A 的刷新消息')).toBeInTheDocument();
  });

  it('restores persisted image previews from thread history after revisiting a conversation', async () => {
    const user = userEvent.setup();
    ...
    const imageThread = createThreadResponse({
      id: 'thread-image-history',
      title: 'image history'
    });
    const otherThread = createThreadResponse({
      id: 'thread-image-other',
      title: 'other history'
    });
    const attachment = {
      id: 'attachment-image',
    ...
    const runtimeFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/codex/status')) return jsonResponse(createCodexStatusResponse());
      if (url.endsWith('/threads?status=active&limit=50')) {
        return jsonResponse({ threads: [imageThread, otherThread] });
      }
      if (url.endsWith(`/threads/${imageThread.id}/history?limit=50`)) {
        return jsonResponse({
          threadId: imageThread.id,
          codexThreadId: 'codex-image-history',
          items: [{
      ...
      if (url.endsWith(`/threads/${otherThread.id}/history?limit=50`)) {
        return jsonResponse({
      ...
      if (url.includes('/runs?limit=50')) return jsonResponse({ runs: [] });
      if (url.endsWith(
        `/attachments/${attachment.id}/content?threadId=${imageThread.id}`
      )) {
        contentRequests += 1;
    ...
    await user.click(await screen.findByRole('button', { name: /image history/ }));
    expect(await screen.findByRole('img', { name: 'screen.png' }))
      .toHaveAttribute('src', 'blob:persisted-image');
    expect(createObjectURL).toHaveBeenCalledTimes(1);
    expect(contentRequests).toBe(1);

    await user.click(screen.getByRole('button', { name: /other history/ }));
    await user.click(screen.getByRole('button', { name: /image history/ }));
    expect(await screen.findByRole('img', { name: 'screen.png' }))
      .toHaveAttribute('src', 'blob:persisted-image');
    expect(contentRequests).toBe(1);
  });

  it('prefetches the enterprise MCP catalog for conversations and refreshes it in Connections', async () => {
    const user = userEvent.setup();
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
    ...
    const threadA = createThreadResponse({
      id: 'thread-connectors-a',
      title: '连接器会话 A'
    });
    const threadB = createThreadResponse({
      id: 'thread-connectors-b',
      title: '连接器会话 B'
    });
    let connectorListRequests = 0;
    let connectorRefreshRequests = 0;
    const runtimeFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = String(input);
      const projectApiResponse = handleDefaultProjectApiRequest(url, init);
      if (projectApiResponse !== undefined) return projectApiResponse;
      ...
      if (url.endsWith('/enterprise/mcp/refresh')) {
        connectorRefreshRequests += 1;
        return jsonResponse({
      ...
      if (url.endsWith('/enterprise/mcp')) {
      ...
      if (url.includes('/history?limit=50')) {
        const threadId = url.includes(threadA.id) ? threadA.id : threadB.id;
    ...
    await screen.findByRole('button', { name: /连接器会话 A/ });
    await waitFor(() => expect(connectorListRequests).toBe(1));
    await user.click(await screen.findByRole('button', { name: /连接器会话 A/ }));
    await user.click(await screen.findByRole('button', { name: /连接器会话 B/ }));
    expect(connectorListRequests).toBe(1);
    expect(connectorRefreshRequests).toBe(0);

    await user.click(screen.getByRole('button', { name: '连接器' }));
    await waitFor(() => expect(connectorRefreshRequests).toBe(1));
    expect(connectorListRequests).toBe(1);
  ...
  it('keeps the current project selected while editing a new schedule', async () => {
    ...
    expect(findPostCall(fetchCalls, '/threads')).toBeUndefined();
    expect(within(screen.getByLabelText('最近会话')).queryByText('任务草稿'))
      .not.toBeInTheDocument();
  });
  ...
  it('uses the centered app confirmation dialog before changing global MCP config', async () => {
    const user = userEvent.setup();
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
    ...
    const runtimeFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/codex/mcp/github') && init?.method === 'PATCH') {
        const body = readRequestBody(init);
        mcpWrites.push(body);
        return jsonResponse({
          server: { ...mcpServer, enabled: body.enabled === true }
        });
      }
      throw new Error(`Unexpected request ${url}`);
    };
    ...
    const connectorToggle = await screen.findByRole('switch', { name: 'github MCP' });
    await user.click(connectorToggle);

    const dialog = screen.getByRole('alertdialog', { name: '确认修改全局配置' });
    expect(dialog).toHaveTextContent('此操作会修改全局 CODEX_HOME，是否继续？');
    expect(mcpWrites).toHaveLength(0);

    await user.click(within(dialog).getByRole('button', { name: '取消' }));
    expect(screen.queryByRole('alertdialog', { name: '确认修改全局配置' }))
      .not.toBeInTheDocument();
    expect(mcpWrites).toHaveLength(0);
    expect(connectorToggle).toHaveAttribute('aria-checked', 'true');

    await user.click(screen.getByRole('button', {
      name: '打开连接器列表，github MCP'
    }));
    await user.click(screen.getByRole('switch', { name: 'github MCP' }));
    await user.click(within(
      screen.getByRole('alertdialog', { name: '确认修改全局配置' })
    ...
    expect(screen.queryByRole('alertdialog', { name: '确认修改全局配置' }))
      .not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: '打开连接器列表，github MCP' }))
      .not.toBeInTheDocument();
  });
  ...
  it('connects the plugin market to real install state and creates draft conversations', async () => {
    ...
    await user.click(within(projectDialog).getByRole('button', { name: '使用所选项目' }));
    ...
    await user.click(within(getSkillMarketCard('frontend-slides')).getByRole('button', { name: '使用' }));
    await user.click(screen.getByRole('button', { name: '使用所选项目' }));
  ...
  it('starts a real runtime run, records SSE events without redundant process detail, and shows Codex info in settings', async () => {
    ...
    expect(screen.getByText('当前 Agent ID')).toBeInTheDocument();
    expect(screen.getByText('clawee_test-agent')).toBeInTheDocument();
  ...
  it.each(['browser', 'desktop'] as const)(
    'creates a conversation inside the Runtime default project on the %s host without sending cwd',
    async kind => {
    const user = userEvent.setup();
    const prompt = 'hi';
    testRuntimeProjects = [];
    expect(window.localStorage.getItem('clawee.preferences.defaultPermission')).toBeNull();
    const hostBridge = createHostBridge();
    hostBridge.kind = kind;
    if (kind === 'desktop') {
    ...
    expect(createThreadBody).not.toHaveProperty('cwd');
    expect(createThreadBody).not.toHaveProperty('workspaceMode');
    const defaultProjectConversations = await screen.findByRole('group', {
      name: '默认项目 会话'
    });
    expect(within(defaultProjectConversations).getByRole('link', {
      name: /hi/
    })).toBeInTheDocument();
    expect(within(screen.getByLabelText('最近会话')).getAllByRole('button', {
      name: /hi/
    })).toHaveLength(1);
    }
  );
  ...
  it.each(['browser', 'desktop'] as const)(
    'pins a conversation through the shared Runtime API in the %s host',
    async (hostKind) => {
      const user = userEvent.setup();
      const hostBridge = createHostBridge();
      hostBridge.kind = hostKind;
      ...
      const fetchCalls: Array<{ url: string; init?: RequestInit }> = [];
      const thread = createThreadResponse({
        id: `thread_pin_${hostKind}`,
        title: `${hostKind} 置顶会话`,
        updatedAt: '2026-08-01T00:00:00.000Z'
      });
      const newerThread = createThreadResponse({
        id: `thread_recent_${hostKind}`,
        title: `${hostKind} 最近会话`,
      ...
      const runtimeFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.endsWith('/codex/status')) return jsonResponse(createCodexStatusResponse());
        if (url.endsWith('/threads?status=active&limit=50')) {
          return jsonResponse({ threads: [thread, newerThread] });
        }
        if (url.endsWith(`/threads/${thread.id}`) && init?.method === 'PATCH') {
          return jsonResponse({
            thread: {
              ...thread,
      ...
      expect(Array.from(recentList.querySelectorAll('.sidebar-recent-row strong'))
        .map(element => element.textContent))
        .toEqual([`${hostKind} 最近会话`, `${hostKind} 置顶会话`]);
      ...
      await waitFor(() => {
        expect(within(conversation).getByRole('button', { name: '取消置顶' }))
          .toBeInTheDocument();
      });
      expect(Array.from(recentList.querySelectorAll('.sidebar-recent-row strong'))
        .map(element => element.textContent))
        .toEqual([`${hostKind} 置顶会话`, `${hostKind} 最近会话`]);
      const patchCall = findPatchCall(fetchCalls, `/threads/${thread.id}`);
      expect(JSON.parse(String(patchCall?.init?.body))).toEqual({ pinned: true });
    }
  );
  ...
  it.each(['browser', 'desktop'] as const)(
    ...
    async (hostKind) => {
      ...
      const archiveDialog = screen.getByRole('alertdialog', { name: '归档会话' });
  ...
  it('adds a local project directory through the browser native Runtime picker', async () => {
    const user = userEvent.setup();
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
    ...
    expect(await screen.findByRole('button', { name: '官网资料' })).toBeInTheDocument();
    expect(screen.queryByRole('dialog', { name: '使用现有文件夹' }))
      .not.toBeInTheDocument();
    expect(readRequestBody(
      findPostCall(fetchCalls, '/projects/directory-selection')!.init!
    )).toEqual({ purpose: 'add' });
    expect(readRequestBody(findPostCall(fetchCalls, '/projects')!.init!)).toEqual({
      cwd: '/Users/test/Documents/官网资料'
    });
  });

  it('uses the browser native Runtime picker when creating a project from a source folder', async () => {
    const user = userEvent.setup();
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
    ...
    expect(await screen.findByText('/Users/test/Projects')).toBeInTheDocument();
    expect(readRequestBody(
      findPostCall(fetchCalls, '/projects/directory-selection')!.init!
    )).toEqual({ purpose: 'create' });
    expect(screen.queryByRole('dialog', { name: '选择源文件夹' }))
      .not.toBeInTheDocument();
  });

  it('persists project order after dragging a sidebar project', async () => {
    const persisted = persistProjects(
      '/Users/test/Documents/content-design',
      '/Users/test/Documents/bili'
    );
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
    ...
    expect(await screen.findByRole('status', { name: '本地运行内核正常' })).toBeInTheDocument();
    const dragged = await screen.findByRole('button', {
    ...
    await waitFor(() => {
      expect(findPostCall(fetchCalls, '/projects/reorder')).toBeDefined();
    });
    expect(readRequestBody(findPostCall(fetchCalls, '/projects/reorder')!.init!))
      .toEqual({ projectIds: [persisted[1]!.id, persisted[0].id] });
    expect(
      Array.from(document.querySelectorAll('.sidebar-project-tree .project-row span'))
        .map(element => element.textContent)
    ).toEqual(['bili', 'content-design']);
  });
  ...
  it('creates a managed project by name through the browser host', async () => {
    ...
    await user.type(screen.getByRole('textbox', { name: '项目名称' }), 'browser-project');
  ...
  it('adds a dropped desktop folder when webkit directory metadata is unavailable', async () => {
    const hostBridge = createHostBridge();
    hostBridge.kind = 'desktop';
  ...
  it('creates a named project from a selected source folder', async () => {
    const user = userEvent.setup();
    const hostBridge = createHostBridge();
    hostBridge.kind = 'desktop';
    hostBridge.selectProjectDirectory = vi.fn(
      async purpose => purpose === 'create'
        ? '/Users/test/Projects'
        : '/Users/test/Documents/existing-project'
    );
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
      token: 'runtime-token'
    ...
    await user.click(screen.getByRole('button', { name: '添加源文件夹' }));
    expect(hostBridge.selectProjectDirectory).toHaveBeenCalledWith('create');
    ...
    expect(hostBridge.selectProjectDirectory).toHaveBeenCalledTimes(2);
    expect(await screen.findByRole('button', {
  ...
  it('loads task summaries without preloading task history or listing task threads as conversations', async () => {
    ...
    expect(within(recentList).getByRole('button', { name: /普通会话/ }))
      .toBeInTheDocument();
    expect(within(recentList).getByRole('button', { name: /^任务草稿 / }))
    ...
    expect(within(recentList).getByRole('button', { name: /^每日总结/ }))
      .toBeInTheDocument();
    expect(
  ...
  it('opens a sidebar task by clearing the previous timeline and delaying task history loading', async () => {
    const user = userEvent.setup();
    ...
    const recentList = await screen.findByLabelText('最近会话');
    await user.click(within(recentList).getByRole('button', { name: /普通会话/ }));
    expect(await screen.findByText('旧会话内容')).toBeInTheDocument();

    await user.click(await within(recentList).findByRole('button', { name: /^每日总结/ }));
  ...
  it('loads and opens a sidebar task whose thread is not in the initial thread list', async () => {
    const user = userEvent.setup();
    const requestedUrls: string[] = [];
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
    ...
    const runtimeFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/schedules')) {
        return jsonResponse({
          schedules: [
            createScheduleResponse({
              id: 'schedule-late-thread',
              threadId: 'thread-late-task',
              name: '刚创建的任务'
            })
          ]
        });
      ...
      if (url.endsWith('/threads/thread-late-task')) {
        return jsonResponse({
          thread: createThreadResponse({
            id: 'thread-late-task',
            title: '刚创建的任务',
            purpose: 'schedule_task',
            scheduleId: 'schedule-late-thread'
          })
        });
      }
      if (url.endsWith('/threads/thread-late-task/history?limit=50')) {
        return jsonResponse({
          threadId: 'thread-late-task',
          codexThreadId: null,
          items: []
      ...
      if (url.endsWith('/threads/thread-late-task/runs?limit=50')) {
        return jsonResponse({ runs: [] });
      }
      throw new Error(`Unexpected request ${url}`);
    };
    ...
    const recentList = await screen.findByLabelText('最近会话');
    await user.click(await within(recentList).findByRole('button', {
      name: /^刚创建的任务/
    }));

    await waitFor(() => {
      expect(window.location.hash).toBe('#/thread/thread-late-task');
    });
    expect(requestedUrls.some(url => url.endsWith('/threads/thread-late-task')))
      .toBe(true);
    expect(await screen.findByRole('heading', { name: '刚创建的任务' }))
      .toBeInTheDocument();
  });
  ...
  it('replaces an uncached transcript and delays the loading state while switching conversations', async () => {
    const user = userEvent.setup();
    const projects = persistProjects(
      '/Users/test/develop/content-design',
      '/Users/test/develop/clawee/bili'
    ...
    const runtimeFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/threads/thread_files/history?limit=50')) {
        return jsonResponse({
          ...
          codexThreadId: null,
          items: [createFileChangeHistoryItem('history-readme', 'README.md')]
        });
      }
  ...
  it.each(['browser', 'desktop'] as const)(
    ...
    const runtimeFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes('/workspace/files/directory?')) {
        ...
        return jsonResponse({
          threadId,
          rootName: 'shared',
          rootPathLabel: '/workspace/shared',
          path: '',
          suggestedOpenPath: 'unrelated.html',
          truncated: false,
          warnings: [],
          nodes: ['A.md', 'B.md', 'unrelated.html'].map(fileName => ({
            path: fileName,
            name: fileName,
            depth: 0,
            type: 'file',
            meta: {
              kind: fileName.endsWith('.html') ? 'html' : 'markdown',
              mime: fileName.endsWith('.html') ? 'text/html' : 'text/markdown',
              size: 1,
              mtimeMs: 1,
              previewable: true,
              editable: true,
    ...
    expect(screen.queryByRole('heading', { name: 'A.md' })).not.toBeInTheDocument();
    expect(screen.queryByRole('heading', { name: 'B.md' })).not.toBeInTheDocument();
    expect(workspaceRequests.some(request => (
      request.includes('threadId=thread_files_empty')
      && request.includes('path=unrelated.html')
    ))).toBe(false);
  ...
  it('shows the Gateway configured model without built-in GPT options', async () => {
    const user = userEvent.setup();
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
    ...
    expect(screen.queryByText(/GPT-5\.6|GPT-5\.5/)).not.toBeInTheDocument();
  ...
  it('migrates the latest conversation model while a new conversation is open', async () => {
    writeRecentModelConfig({
      model: 'gpt-5.5',
      reasoning: 'low'
    });
    ...
    const runtimeFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/threads/thread_selected_model/history?limit=50')) {
        return jsonResponse({
          threadId: thread.id,
          codexThreadId: null,
          items: []
      ...
      if (url.endsWith('/threads/thread_selected_model/runs?limit=50')) {
        return jsonResponse({ runs: [] });
      }
      throw new Error(`Unexpected request ${url}`);
    };
  ...
  it('prefers the global model selection over project model config', async () => {
    const user = userEvent.setup();
    writeRecentModelConfig({
      model: 'gpt-5.5',
      reasoning: 'xhigh'
  ...
  it('uses the global model selection when sending in an existing conversation', async () => {
    const user = userEvent.setup();
    const prompt = '继续处理这个会话';
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({ baseUrl: 'http://127.0.0.1:60764', token: 'runtime-token' });
    const fetchCalls: Array<{ url: string; init?: RequestInit }> = [];
    const existingThread = createThreadResponse({
      id: 'thread_existing',
      title: '已有会话',
      sandbox: 'read-only',
      model: 'gpt-5.6-sol',
  ...
  it('applies one model selection to new and existing conversations globally', async () => {
    const user = userEvent.setup();
    const hostBridge = createHostBridge();
    hostBridge.readConnectionConfig = async () => ({
      baseUrl: 'http://127.0.0.1:60764',
    ...
    const runtimeFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (/\/threads\/thread_(first|second)\/history\?limit=50$/.test(url)) {
        const threadId = url.includes('thread_first') ? 'thread_first' : 'thread_second';
        return jsonResponse({ threadId, codexThreadId: null, items: [] });
      }
      if (/\/threads\/thread_(first|second)\/runs\?limit=50$/.test(url)) {
        return jsonResponse({ runs: [] });
      }
      throw new Error(`Unexpected request ${url}`);
    };
...
function handleDefaultProjectApiRequest(
  ...
  if (
    new URL(url).pathname.endsWith('/projects/directory-selection')
    && init?.method === 'POST'
  ) {
    const purpose = readRequestBody(init).purpose;
    return jsonResponse({
      path: purpose === 'create'
        ? '/Users/test/Projects'
        : '/Users/test/Documents/官网资料'
    });
  }
  ...
  if (url.endsWith('/projects/reorder') && init?.method === 'POST') {
    const body = readRequestBody(init);
    const projectIds = Array.isArray(body.projectIds)
      ? body.projectIds.filter((projectId): projectId is string => typeof projectId === 'string')
      : [];
    const current = readTestRuntimeProjects();
    const projectsById = new Map(current.map(project => [project.id, project]));
    const activeProjects = projectIds.flatMap(projectId => {
      const project = projectsById.get(projectId);
      return project === undefined ? [] : [project];
    });
    testRuntimeProjects = [
      ...activeProjects,
      ...current.filter(project => project.status === 'archived')
    ];
    return jsonResponse({ projects: activeProjects });
  }
  if (url.endsWith('/projects/managed') && init?.method === 'POST') {
    const body = readRequestBody(init);
    const name = typeof body.name === 'string' ? body.name.trim() : 'project';
    const parentCwd = typeof body.parentCwd === 'string'
      ? body.parentCwd
      : '/Users/test/Documents/Clawee';
    const project = createTestProject(`${parentCwd}/${name}`, { name });
    testRuntimeProjects = [
      project,
...
function createActivityStatisticsResponse(
...
function createActivityDetailResponse(input: {
  collectorId: string;
  agentId: string;
}): EnterpriseActivityDetailResponse {
  return {
...
async function showSkillMarketCard(
  ...
  const publicMarketTab = await screen.findByRole('tab', { name: 'Skill市场' });
  if (publicMarketTab.getAttribute('aria-selected') !== 'true') {
    await user.click(publicMarketTab);
  }
...
function createFileChangeHistoryItem(id: string, path: string): ThreadHistoryItem {
  return {
    id,
    type: 'file_change',
