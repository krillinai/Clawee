export type AppRoute =
  ...
  | { view: 'activity-recharge-records'; range: ActivityRange }
...
export function parseRoute(hash: string): AppRoute {
  ...
  if (path === '#/activity/recharge-records') {
    return { view: 'activity-recharge-records', range: parseActivityRange(query) };
  }
...
export function formatRoute(route: AppRoute): string {
  switch (route.view) {
    ...
    case 'activity-recharge-records':
      return `#/activity/recharge-records?range=${route.range}`;
    case 'plugins':
