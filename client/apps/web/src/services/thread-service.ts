...
export function createThreadService(client: RuntimeClient) {
  return {
    ...
    listThreadRuns(
      ...
      return getWithSignal(
        client,
        `/threads/${encodeURIComponent(threadId)}/runs?limit=50`,
        options.signal
      );
    },
    getThreadHistory(
      ...
      return getWithSignal(
        client,
        `/threads/${encodeURIComponent(threadId)}/history${suffix}`,
        options.signal
      );
    }
  };
