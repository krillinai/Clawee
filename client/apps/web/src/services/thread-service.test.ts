...
describe('ThreadService', () => {
  ...
  it('passes cancellation signals to thread history and run requests', async () => {
    const get = vi.fn(async (_path: string, _options?: { signal?: AbortSignal }) => ({
      threadId: 'thread_1',
      items: [],
      runs: []
    }));
    const client = {
      get<T>(path: string, options?: { signal?: AbortSignal }): Promise<T> {
        return get(path, options) as Promise<T>;
      }
    } as RuntimeClient;
    const service = createThreadService(client);
    const controller = new AbortController();

    await service.getThreadHistory('thread_1', { limit: 50 }, {
      signal: controller.signal
    });
    await service.listThreadRuns('thread_1', {
      signal: controller.signal
    });

    expect(get).toHaveBeenNthCalledWith(
      1,
      '/threads/thread_1/history?limit=50',
      { signal: controller.signal }
    );
    expect(get).toHaveBeenNthCalledWith(
      2,
      '/threads/thread_1/runs?limit=50',
      { signal: controller.signal }
    );
  });
