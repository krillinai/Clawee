...
打开 `http://127.0.0.1:19860/`。首次访问会按需启动 daemon；页面顶部显示“本地运行内核正常”后即可发送任务。
...
4. 不同会话最多同时运行 10 个；同一会话内仍按发送顺序串行执行。
5. 并行会话达到 10 个后，新任务会明确提示正在等待，并在任一执行位置释放后自动开始。
6. 切换会话不会停止后台 Run；侧栏转圈表示该会话仍在执行。
7. 任务中心可查看运行中、失败、完成和待审批任务，并跳转到 Run Detail。
...
2. 请求 `http://127.0.0.1:19860/.clawee/runtime/healthz`，应返回 `{"ok":true}`。
3. 检查终端是否出现 `RUNTIME_START_FAILED` 或 `RUNTIME_PROXY_FAILED`。
4. 确认 19860 端口没有被旧进程占用；该端口被占用时 Vite 会直接报错退出。
