...
export type ProjectDirectorySelectionPurpose = 'add' | 'create' | 'replace';
...
export type RuntimeHealthResponse = {
  ok: true;
  runtimeState?: 'configuration_required' | 'ready';
};

export type ModelServiceConfiguration = {
  baseUrl: string;
  model: string;
};

export type ModelAccessMode = 'platform_managed' | 'enterprise_managed';

export type ModelAccessState =
  | { status: 'resolving' }
  | { status: 'configuration_required'; mode: 'enterprise_managed' }
  | {
      status: 'ready';
...
export type ModelServiceConfigurationStatusResponse = {
  status: 'configuration_required' | 'ready';
  configuration: ModelServiceConfiguration | null;
  apiKeyConfigured: boolean;
};

export type ConfigureModelServiceRequest = ModelServiceConfiguration & {
  apiKey: string;
};

export type ConfigureModelServiceResponse = {
  status: 'ready';
  configuration: ModelServiceConfiguration;
...
export type CodexRuntimeStatus = {
  ...
  activationState: CodexRuntimeActivationState;
...
export type SelectProjectDirectoryRequest = {
...
export type EnterpriseDingTalkLoginPrepareResponse = {
  authorizationUrl: string;
  expiresAt: string;
};

export type EnterpriseActivityRange = 'today' | '7d' | '30d';

export type EnterpriseBusinessDataStatus =
  | 'unconfigured'
  | 'available'
  | 'partial'
  | 'unavailable';
...
export type EnterpriseBillingOverviewResponse = {
  balanceCny: number;
  generatedAt: string;
};

export type EnterpriseRechargeSessionResponse = {
  rechargeUrl: string;
};

export type EnterpriseRechargeOrderStatus =
  | 'pending_payment'
  | 'crediting'
  | 'succeeded'
  | 'credit_failed'
...
export type EnterpriseRechargeOrder = {
  orderNo: string;
  amountCents: number;
  currency: 'CNY';
  channel: 'alipay' | 'manual';
  status: EnterpriseRechargeOrderStatus;
...
export type EnterpriseRechargeOrderPageResponse = {
  items: EnterpriseRechargeOrder[];
  page: number;
  total: number;
};

export type EnterpriseActivityCapabilityResponse = {
  allowed: boolean;
  refreshedAt: string;
};

export type EnterpriseActivityTokenUsage = {
  inputTokens: number;
...
export type EnterpriseActivityUsageDistributionItem = {
  id: string;
  label: string;
...
export type EnterpriseActivityModelDistributionItem = {
  model: string;
  requests: number;
  inputTokens: number;
  cachedInputTokens: number;
...
export type EnterpriseActivityTokenUsageRank = {
  rank: number;
  name: string;
  requests: number;
  totalTokens: number;
...
export type EnterpriseActivityTrendPoint = EnterpriseActivityTokenUsage & {
  bucketStart: string;
};

export type EnterpriseActivityAgentSummary = {
  collectorId: string;
  agentId: string;
  name: string;
  status: string;
...
export type EnterpriseActivityStatisticsResponse = {
  ...
  generatedAt: string;
  organization: {
    usage: EnterpriseActivityTokenUsage;
    activeEmployees: number;
    activeAgents: number;
    completedTurns: number;
    mcpDistribution: EnterpriseActivityUsageDistributionItem[];
  };
  trend: {
    granularity: 'hour' | 'day';
    points: EnterpriseActivityTrendPoint[];
  };
  modelDistribution: EnterpriseActivityModelDistributionItem[];
  tokenUsageRanking: EnterpriseActivityTokenUsageRank[];
  agents: EnterpriseActivityAgentSummary[];
  dataStatus: {
    sub2api: string;
    activity: string;
  };
...
export type EnterpriseActivitySession = {
  sessionId?: string;
  title?: string;
  status?: string;
  summary?: string;
...
export type EnterpriseActivityTurn = {
  turnId?: string;
  sessionId?: string;
  title?: string;
  status?: string;
...
export type EnterpriseActivitySubAgent = {
  subAgentId?: string;
  name?: string;
  status?: string;
  completedTurns?: number;
...
export type EnterpriseActivityToolCall = {
  toolCallId?: string;
  name?: string;
  type?: string;
  status?: string;
...
export type EnterpriseActivityStatusTimelineItem = {
  status?: string;
  occurredAt?: string;
  message?: string;
};

export type EnterpriseActivityRecentItem = {
  activityId?: string;
  type?: string;
  title?: string;
  status?: string;
...
export type EnterpriseActivityDetailResponse = {
  ...
  sessions: EnterpriseActivitySession[];
  turns: EnterpriseActivityTurn[];
  subAgents: EnterpriseActivitySubAgent[];
  toolCalls: EnterpriseActivityToolCall[];
  statusTimeline: EnterpriseActivityStatusTimelineItem[];
  recentActivities: EnterpriseActivityRecentItem[];
  stats: {
...
export type CodexMcpPresetResponse = {
  name: string;
  displayName: string;
  description: string;
  provider: string;
...
export type CodexMcpListResponse = {
  ...
  presets: CodexMcpPresetResponse[];
  diagnostics: string[];
};
