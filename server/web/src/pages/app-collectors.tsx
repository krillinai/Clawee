  ...
  return (
    <PageShell>
      ...
      <CollectorRegistrationPanel
        createError={createRegistration.error}
        loadError={registrationQuery.error}
      ...
      <DataTableShell dense minWidth={1040}>
        ...
        <TableBody>
          ...
          {query.isError ? <TableStateRow colSpan={8} tone="danger"><ErrorAlert error={query.error}>Collector 加载失败</ErrorAlert></TableStateRow> : null}
  ...
  return (
    <Card className="min-w-0 shadow-none">
      ...
      <CardContent className="flex min-w-0 flex-col gap-4">
        {createError ? <ErrorAlert error={createError}>生成采集器注册码失败，请稍后重试。</ErrorAlert> : null}
        {loadError ? <ErrorAlert error={loadError}>加载采集器注册码失败，请稍后重试。</ErrorAlert> : null}
...
function CollectorDetail({ collectorId }: { collectorId: string }) {
  ...
  if (query.isError || !query.data) return <PageShell><ErrorAlert error={query.error}>Collector 不存在或无权访问</ErrorAlert></PageShell>;
  ...
  return (
    <PageShell>
      ...
      {revoke.isError ? <ErrorAlert error={revoke.error}>Token 撤销失败</ErrorAlert> : null}
