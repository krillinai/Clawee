...
describe("客户端下载页", () => {
  ...
  it("清单请求失败时显示错误且不展示下载链接", async () => { downloadMock.mockRejectedValue(new Error("offline")); showPage(); expect(await screen.findByText(/客户端发布清单暂时不可用/)).toBeInTheDocument(); expect(screen.queryByRole("link", { name: "下载安装包" })).not.toBeInTheDocument(); });
  it("展示三平台包、签名和校验值，并复制服务端地址", async () => {
    ...
    expect(await screen.findByText("macOS Apple Silicon")).toBeInTheDocument(); expect(screen.getByText("macOS Intel")).toBeInTheDocument(); expect(screen.getByText("Windows x64")).toBeInTheDocument(); expect(screen.getByText(/0.1.7 · 未签名/)).toBeInTheDocument(); expect(screen.getAllByRole("link", { name: "下载安装包" }).map((link) => link.getAttribute("href"))).toEqual(packages.map((item) => item.download_url)); fireEvent.click(screen.getByRole("button", { name: "复制" })); await vi.waitFor(() => expect(writeText).toHaveBeenCalledWith(data.gateway_url));
  });
