...
describe("客户端下载页", () => {
  ...
  it("展示三平台包和校验值，并复制服务端地址", async () => {
    ...
    expect(await screen.findByText("macOS Apple Silicon")).toBeInTheDocument(); expect(screen.getByText("macOS Intel")).toBeInTheDocument(); expect(screen.getByText("Windows")).toBeInTheDocument(); expect(screen.getByText("客户端首次使用时，在登录页面的服务端地址设置中填入此地址以连接企业服务。")).toBeInTheDocument(); expect(screen.getAllByText("0.1.7")).toHaveLength(3); expect(screen.queryByText("未签名")).not.toBeInTheDocument(); expect(screen.queryByText("已签名并公证")).not.toBeInTheDocument(); expect(screen.getAllByRole("link", { name: "下载安装包" }).map((link) => link.getAttribute("href"))).toEqual(packages.map((item) => item.download_url)); fireEvent.click(screen.getByRole("button", { name: "复制" })); await vi.waitFor(() => expect(writeText).toHaveBeenCalledWith(data.gateway_url));
  });
