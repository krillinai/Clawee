...
TEST_DATABASE_URL="${CLAW_GATEWAY_TEST_DATABASE_URL:-postgres://claw_gateway@localhost:5933/claw_gateway_test?sslmode=disable}"
...
if [[ -z "${CLAW_GATEWAY_TEST_DATABASE_URL:-}" ]]; then
  ...
  for _ in {1..30}; do
    if docker compose -f "$TEST_COMPOSE_FILE" exec -T postgres-test pg_isready -U claw_gateway -d claw_gateway >/dev/null 2>&1; then
      break
    fi
    sleep 1
  done
