...
func mountSharedFileRoutes(app, admin *gin.RouterGroup, opts Options) {
	...
	admin.PUT("/shared-spaces", requirePermission(opts.RBACService, rbac.PermissionSharedFilesSpaceUpdate), handleAdminUpdateSharedSpace(opts.SharedFilesService))
	...
	admin.PUT("/shared-spaces/account-grants", requirePermission(opts.RBACService, rbac.PermissionSharedFilesMemberUpdate), handleAdminUpdateSharedMember(opts.SharedFilesService))
